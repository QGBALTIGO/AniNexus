...
export async function syncClerkUser(clerkUser) {
  ...
  const { rows } = await q(`
    INSERT INTO users(email,username,password_hash,clerk_user_id,display_name,avatar_url,email_verified,last_login_at,last_seen_at,role)
    ...
    ON CONFLICT(email) DO UPDATE SET
      ...
      updated_at=now()
    RETURNING *
  `, [emailState.address, username, clerkUser.id, displayName, avatarUrl, emailState.verified, ADMIN_EMAILS.has(emailState.address)?'admin':'user']);
  const user = rows[0];
...
async function clerkCurrentUser(request) {
  ...
  const { rows } = await q(`SELECT id,email,username,display_name,role,status,suspended_until,ban_reason,avatar_url,bio,theme,privacy,email_verified,created_at,clerk_user_id
...
async function legacyCurrentUser(request) {
  ...
  if (cached) return accountAccess(cached,request);
  const { rows } = await q(`SELECT u.id,u.email,u.username,u.display_name,u.role,u.status,u.suspended_until,u.ban_reason,u.avatar_url,u.bio,u.theme,u.privacy,u.email_verified,u.created_at,u.clerk_user_id
