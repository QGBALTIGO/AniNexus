...
const sourceRewardSchema=z.object({
  coins:z.number().int().min(0),
  dados:z.number().int().min(0),
  claimed:z.boolean(),
  available:z.boolean(),
  claimedAt:z.string().max(80).nullable().optional(),
  coinsGranted:z.number().int().min(0).optional(),
  dadosGranted:z.number().int().min(0).optional(),
}).optional();
const sourceIntegrationSchema=z.object({
  linked:z.boolean(),
  linkedAt:z.string().max(80).nullable().optional(),
  updatedAt:z.string().max(80).nullable().optional(),
  badge:z.string().max(80).nullable().optional(),
  reward:sourceRewardSchema,
}).optional();
...
const profileSchema=z.object({
  ...
  public:z.boolean(),integration:sourceIntegrationSchema,updatedAt:z.string().min(1).max(80),
});
const consumeSchema=z.object({
  linkId:z.string().uuid(),sourceSubject:z.string().regex(/^src_[a-f0-9]{64}$/),revokeToken:z.string().min(20).max(256),reward:sourceRewardSchema,profile:profileSchema,
});
