...
async function gql(query, variables={}) {
  ...
  try {
    const res = await fetch(ENDPOINT,{method:'POST',headers:{'content-type':'application/json','accept':'application/json','user-agent':'AniNexus/1.1'},body:JSON.stringify({query,variables}),signal:ctl.signal,redirect:'error'});
...
function normalizeMedia(m) {
  ...
  const streaming = (m.externalLinks||[]).filter(x=>String(x.type||'').toUpperCase()==='STREAMING').map(x=>({site:String(x.site||'').slice(0,80),url:safeHttpsUrl(x.url),icon:safeHttpsUrl(x.icon),color:String(x.color||'').slice(0,24)})).filter(x=>x.url);
  const base = {
    id:m.id, slug:`${slugify(title)}-${m.id}`, title:String(title).slice(0,300), titleRomaji:String(m.title?.romaji||'').slice(0,300), titleNative:String(m.title?.native||'').slice(0,300),
    ...
    description:strip(m.description), genres:(m.genres||[]).slice(0,30).map(x=>String(x).slice(0,80)), tags:(m.tags||[]).slice(0,12).map(t=>String(t.name||'').slice(0,120)), score:m.averageScore ? +(m.averageScore/10).toFixed(1) : null,
...
const MANGA_FIELDS = `id title{romaji english native} coverImage{extraLarge large color} bannerImage description genres tags{name} averageScore popularity favourites chapters volumes format status countryOfOrigin source startDate{year month day} endDate{year month day} externalLinks{site url type icon color}`;
...
export async function getCatalog(opts={}) {
  ...
  return cacheRemember(key, opts.search?60:300, async()=>{
    ...
    const data=await gql(query,{page,perPage,search:opts.search?String(opts.search).slice(0,120):null,genre:opts.genre?String(opts.genre).slice(0,80):null,format:opts.format||null,season:opts.season||null,year:opts.year?Number(opts.year):null,sort:sortMap[opts.sort]||['POPULARITY_DESC']});
...
export async function getReading(opts={}) {
  ...
  return cacheRemember(key,opts.search?90:600,async()=>{
    ...
    const sortMap={POPULAR:['POPULARITY_DESC'],SCORE:['SCORE_DESC'],TRENDING:['TRENDING_DESC'],NEW:['START_DATE_DESC'],TITLE:['TITLE_ROMAJI']};
    const data=await gql(query,{page,perPage,search:opts.search?String(opts.search).slice(0,120):null,format,sort:sortMap[opts.sort]||['POPULARITY_DESC']});
...
export async function getAnime(id) {
  ...
  return cacheRemember(`anime:${mediaId}`, 600, async()=>{
    ...
    const a=rows[0]||{};
    const curatedStreaming=Array.isArray(a.streaming)?a.streaming.map(x=>({site:String(x?.site||'').slice(0,80),url:safeHttpsUrl(x?.url),icon:safeHttpsUrl(x?.icon),color:String(x?.color||'').slice(0,24)})).filter(x=>x.url):[];
    return {...base,title:a.title_pt_br?String(a.title_pt_br).slice(0,300):base.title,description:a.synopsis_pt_br?String(a.synopsis_pt_br).slice(0,30_000):base.description,dubbed:!!a.dubbed_pt_br,subtitled:!!a.subtitle_pt_br,streaming:curatedStreaming.length?curatedStreaming:base.streaming,
      ...
      staff:(m.staff?.edges||[]).slice(0,20).map(e=>({role:String(e.role||'').slice(0,180),id:e.node.id,name:String(e.node.name?.full||'').slice(0,180),image:safeHttpsUrl(e.node.image?.large||e.node.image?.medium)})),
...
export async function getStudios(page=1){
  ...
  return cacheRemember(`studios:${p}`,3600,async()=>{const query=`query($page:Int){Page(page:$page,perPage:30){pageInfo{total currentPage lastPage hasNextPage} studios(sort:FAVOURITES_DESC){id name isAnimationStudio favourites media(perPage:6,sort:POPULARITY_DESC){nodes{${MEDIA_FIELDS}}}}}}`;const d=await gql(query,{page:p});return {pageInfo:d.Page.pageInfo,items:d.Page.studios.filter(s=>s.isAnimationStudio).map(s=>({...s,name:String(s.name||'').slice(0,180),media:(s.media?.nodes||[]).map(normalizeMedia)}))};});
}

export async function getDubbed(page=1){
  const p=Math.max(1,Math.min(1000,Number(page)||1));
  const {rows}=await q(`SELECT m.payload,a.dubbed_pt_br,a.subtitle_pt_br,a.streaming FROM media_annotations a JOIN media_cache m ON m.media_id=a.media_id WHERE a.dubbed_pt_br=true ORDER BY m.updated_at DESC LIMIT 24 OFFSET $1`,[(p-1)*24]);
  return {items:rows.map(r=>({...r.payload,dubbed:r.dubbed_pt_br,subtitled:r.subtitle_pt_br,streaming:Array.isArray(r.streaming)&&r.streaming.length?r.streaming:r.payload.streaming})),pageInfo:{currentPage:p,hasNextPage:rows.length===24}};
}
