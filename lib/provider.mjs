...
function queueMediaCache(base) {
  ...
  mediaQueue.set(`${mediaType}:${base.id}`,{media_type:mediaType,media_id:base.id,slug:base.slug,payload:base});
...
function normalizeMedia(m, mediaType='ANIME') {
  ...
  const base = {
    id:m.id,idMal:m.idMal||null,mediaType:mediaType==='MANGA'?'MANGA':'ANIME',slug:`${slugify(title)}-${m.id}`,title:String(title).slice(0,300),titleRomaji:String(m.title?.romaji||'').slice(0,300),titleNative:String(m.title?.native||'').slice(0,300),synonyms:(m.synonyms||[]).slice(0,30).map(x=>String(x).slice(0,300)),
...
const CATALOG_FALLBACK_SORT={
  POPULAR:`CASE WHEN payload->>'popularity' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'popularity')::numeric ELSE 0 END DESC,updated_at DESC`,
  SCORE:`CASE WHEN payload->>'score' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'score')::numeric ELSE 0 END DESC,CASE WHEN payload->>'popularity' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'popularity')::numeric ELSE 0 END DESC`,
  TRENDING:`updated_at DESC,CASE WHEN payload->>'popularity' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'popularity')::numeric ELSE 0 END DESC`,
  NEW:`updated_at DESC`,
  TITLE:`lower(COALESCE(payload->>'title','')) ASC`,
  FAVOURITES:`CASE WHEN payload->>'favourites' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'favourites')::numeric ELSE 0 END DESC,CASE WHEN payload->>'popularity' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'popularity')::numeric ELSE 0 END DESC`,
  MATCH:`CASE WHEN payload->>'popularity' ~ '^[0-9]+([.][0-9]+)?$' THEN (payload->>'popularity')::numeric ELSE 0 END DESC,updated_at DESC`,
};
...
async function cachedCatalog(opts,page,perPage,status){
  ...
  const order=CATALOG_FALLBACK_SORT[String(opts.sort||'').toUpperCase()]||CATALOG_FALLBACK_SORT.POPULAR;
  const offset=(page-1)*perPage;
  const {rows}=await q(`SELECT payload,count(*) OVER()::int AS total
    FROM media_cache
    WHERE media_type='ANIME'
      AND COALESCE(payload->>'mediaType','ANIME')='ANIME'
      AND COALESCE(payload->>'format','') NOT IN ('MANGA','NOVEL','ONE_SHOT')
      AND ($1::text IS NULL OR payload->>'status'=$1)
      AND ($2::text IS NULL OR payload->>'format'=$2)
      AND ($3::text IS NULL OR payload->>'season'=$3)
      AND ($4::int IS NULL OR CASE WHEN payload->>'seasonYear' ~ '^[0-9]{4}$' THEN (payload->>'seasonYear')::int END=$4)
      AND ($5::text IS NULL OR payload->'genres' @> to_jsonb(ARRAY[$5]::text[]))
      AND ($6::text IS NULL OR lower(COALESCE(payload->>'title','')||' '||COALESCE(payload->>'titleRomaji','')||' '||COALESCE(payload->>'titleNative','')) LIKE '%'||lower($6)||'%')
    ORDER BY ${order}
    LIMIT $7 OFFSET $8`,[status,format,season,year,genre,search,perPage,offset]).catch(()=>({rows:[]}));
...
export async function getMediaSummaries(values=[], mediaType='ANIME') {
  ...
  return type==='ANIME'?annotateMedia(items):items;
}
...
export async function getCatalog(opts={}) {
  ...
  const key='catalog:v2:'+JSON.stringify({...opts,page,perPage,status});
  return cacheRemember(key, opts.search?60:300, async()=>{
    try{
      ...
      const result={pageInfo:data.Page.pageInfo,items:await annotateMedia(data.Page.media.map(normalizeMedia))};
...
export async function getReading(opts={}) {
  ...
  return cacheRemember(key,opts.search?90:600,async()=>{
    ...
    return {pageInfo:data.Page.pageInfo,items:data.Page.media.map(m=>{const b=normalizeMedia(m,'MANGA');return {...b,chapters:m.chapters||null,volumes:m.volumes||null}})};
  },{staleTtl:3600});
}
...
export async function getManga(id) {
  ...
  return cacheRemember(`manga:${mediaId}`,900,async()=>{
    ...
    const d=await gql(query,{id:mediaId}),m=d.Media,b=normalizeMedia(m,'MANGA');
    return {...b,chapters:m.chapters||null,volumes:m.volumes||null,relations:(m.relations?.edges||[]).slice(0,30).map(e=>({relationType:e.relationType,media:{...normalizeMedia(e.node,'MANGA'),chapters:e.node.chapters||null,volumes:e.node.volumes||null}})),recommendations:(m.recommendations?.nodes||[]).slice(0,20).map(n=>({rating:n.rating,media:n.mediaRecommendation?{...normalizeMedia(n.mediaRecommendation,'MANGA'),chapters:n.mediaRecommendation.chapters||null,volumes:n.mediaRecommendation.volumes||null}:null})).filter(x=>x.media)};
  },{staleTtl:86400});
}
...
export async function getSchedule(start,end) {
  ...
  const key=`schedule:v2:${bucketStart}:${bucketEnd}`;
