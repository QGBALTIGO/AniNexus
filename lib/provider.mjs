...
async function enrichFromJikan(base,mediaType='ANIME'){
  ...
  if(!Number.isSafeInteger(malId)||malId<=0){
    const similar=await similarCachedMedia(base,type);return{...base,recommendations:similar.map(media=>({rating:0,media}))};
  }
  return cacheRemember(`jikan-detail:v2:${type}:${malId}`,21600,async()=>{
    const prefix=type==='MANGA'?'manga':'anime';
    const [fullResult,charactersResult,recommendationsResult]=await Promise.allSettled([
      jikanJson(`/${prefix}/${malId}/full`),jikanJson(`/${prefix}/${malId}/characters`),jikanJson(`/${prefix}/${malId}/recommendations`)
    ]);
    ...
    const companies=type==='ANIME'?[...(full?.studios||[]).map(row=>({...row,role:'Estúdio'})),...(full?.producers||[]).map(row=>({...row,role:'Produção'})),...(full?.licensors||[]).map(row=>({...row,role:'Licenciamento'}))]:[...(full?.authors||[]).map(row=>({...row,role:String(row?.type||'Autoria')}))];
    const staff=companies.slice(0,20).map(row=>({role:row.role,id:Number(row.mal_id)||0,name:String(row.name||'').slice(0,180),native:'',image:''})).filter(row=>row.name);
  ...
  },{staleTtl:604800}).then(extra=>({...base,...extra}));
}
...
export async function getManga(id) {
  const mediaId=Number(id);if(!Number.isSafeInteger(mediaId)||mediaId<=0)throw new Error('invalid media id');
  return cacheRemember(`manga:v2:${mediaId}`,900,async()=>{
    try{
...
export async function getAnime(id) {
  const mediaId=Number(id);if(!Number.isSafeInteger(mediaId)||mediaId<=0)throw new Error('invalid media id');
  return cacheRemember(`anime:v2:${mediaId}`,600,async()=>{
    try{
