...
async function enrichFromJikan(base,mediaType='ANIME'){
  ...
  if(!Number.isSafeInteger(malId)||malId<=0){
    const similar=await similarCachedMedia(base,type),out={...base,rosterVersion:6,staff:mergeRoster(usableCachedStaff(base.staff),[]),recommendations:similar.map(media=>({rating:0,media}))};queueMediaCache(out);return out;
  }
...
export async function getManga(id) {
  const mediaId=Number(id);if(!Number.isSafeInteger(mediaId)||mediaId<=0)throw new Error('invalid media id');
  return cacheRemember(`manga:v7:${mediaId}`,900,async()=>{
    try{
...
export async function getAnime(id) {
  const mediaId=Number(id);if(!Number.isSafeInteger(mediaId)||mediaId<=0)throw new Error('invalid media id');
  return cacheRemember(`anime:v7:${mediaId}`,600,async()=>{
    try{
