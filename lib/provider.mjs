...
import { fetchAnimeThemes } from './anime-themes.mjs';
...
export async function getAnimeThemes(id){
  ...
  return cacheRemember(`anime-themes:${mediaId}`,6*3600,async()=>{
    return normalizeAnimeThemes(await fetchAnimeThemes(mediaId,{endpoint:ANIMETHEMES_ENDPOINT,timeout:Math.min(timeoutMs,8000)}));
  },{staleTtl:3*86400});
}
...
export function catalogPageInfo(info={},page=1){
  // A capped provider total is not an exact count of search results.
  if(info.hasNextPage===true&&Number(info.total)>=5000)return {...info,currentPage:Number(info.currentPage)||page,total:null,lastPage:null,totalIsEstimated:true};
  return info;
}
...
export async function getCatalog(opts={}) {
  ...
  const communityOnly=opts.communityOnly===true||String(opts.communityOnly||'')==='1';
  const key='catalog:v8:'+JSON.stringify({...opts,page,perPage,status,sort,communityOnly});
  return cacheRemember(key, opts.search?60:300, async()=>{
    ...
    try{
      ...
      const result={pageInfo:catalogPageInfo(data.Page.pageInfo,page),items:sortByInternalMetrics(await annotateMedia(visible,'ANIME'),sort)};
...
export async function getReading(opts={}) {
  ...
  const sort=requestedSort(opts.sort),metricSort=['POPULAR','SCORE','TRENDING','FAVOURITES','MEMBERS'].includes(sort);
  const communityOnly=opts.communityOnly===true||String(opts.communityOnly||'')==='1';
  const key='reading:v6:'+JSON.stringify({...opts,page,perPage,format,status,sort,communityOnly});
  return cacheRemember(key,opts.search?90:600,async()=>{
    ...
    try{
      ...
      const visible=requestedDiscoveryMinimum(opts.discover)>0?normalized.filter(isPresentableDiscoveryMedia):normalized;
      const result={pageInfo:catalogPageInfo(data.Page.pageInfo,page),items:sortByInternalMetrics(await annotateMedia(visible,'MANGA'),sort)};
