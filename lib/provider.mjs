...
async function enrichFromJikan(base,mediaType='ANIME'){
  ...
  const [fullResult,charactersResult,staffResult,recommendationsResult]=await Promise.allSettled([
    ...
    const kitsu=await kitsuPromise,baseStaff=usableCachedStaff(base.staff);
    const out={...base,rosterVersion:2,
      characters:mergeRoster(characters,mergeRoster(base.characters,kitsu.characters)),
      staff:mergeRoster(staff,mergeRoster(baseStaff,kitsu.staff)),
