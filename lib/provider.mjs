...
const CATALOG_FALLBACK_SORT={
  POPULAR:dir=>`(COALESCE(stats.list_count,0)+COALESCE(fav.favorite_count,0)*3+COALESCE(imp.impression_count,0)*2) ${dir},mc.updated_at DESC`,
  SCORE:dir=>`COALESCE(stats.score,0) ${dir},COALESCE(stats.rating_count,0) ${dir},mc.updated_at DESC`,
  TRENDING:dir=>`GREATEST(stats.last_activity,fav.last_activity,imp.last_activity) ${dir} NULLS LAST,mc.updated_at DESC`,
  NEW:dir=>`mc.updated_at ${dir}`,
  TITLE:dir=>`lower(COALESCE(mc.payload->>'title','')) ${dir},mc.updated_at DESC`,
  FAVOURITES:dir=>`COALESCE(fav.favorite_count,0) ${dir},COALESCE(stats.list_count,0) ${dir},mc.updated_at DESC`,
  MEMBERS:dir=>`COALESCE(stats.list_count,0) ${dir},COALESCE(fav.favorite_count,0) ${dir},mc.updated_at DESC`,
  MATCH:dir=>`(COALESCE(stats.list_count,0)+COALESCE(fav.favorite_count,0)*3+COALESCE(imp.impression_count,0)*2) ${dir},mc.updated_at DESC`,
};
...
export async function getCatalog(opts={}) {
  ...
  const communityOnly=opts.communityOnly===true||String(opts.communityOnly||'')==='1';
  const key='catalog:v4:'+JSON.stringify({...opts,page,perPage,status,sort,communityOnly});
...
export async function getReading(opts={}) {
  ...
  const sort=requestedSort(opts.sort),metricSort=['POPULAR','SCORE','TRENDING','FAVOURITES','MEMBERS'].includes(sort);
  const key='reading:v3:'+JSON.stringify({...opts,page,perPage,format,sort});
