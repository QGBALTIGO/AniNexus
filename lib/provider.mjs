...
function sortByInternalMetrics(items=[],sort='POPULAR'){
  ...
  if(!['POPULAR','SCORE','TRENDING','FAVOURITES'].includes(mode))return items;
  const rows=items.map((item,index)=>({item,index}));
  rows.sort((a,b)=>{
    const av=mode==='SCORE'?Number(a.item.score??-1):mode==='FAVOURITES'?Number(a.item.favourites||0):Number(a.item.popularity||0);
    const bv=mode==='SCORE'?Number(b.item.score??-1):mode==='FAVOURITES'?Number(b.item.favourites||0):Number(b.item.popularity||0);
...
const CATALOG_FALLBACK_SORT={
  POPULAR:`(COALESCE(stats.list_count,0)+COALESCE(fav.favorite_count,0)*3+COALESCE(imp.impression_count,0)*2) DESC,mc.updated_at DESC`,
  SCORE:`COALESCE(stats.score,0) DESC,COALESCE(stats.rating_count,0) DESC,mc.updated_at DESC`,
  TRENDING:`GREATEST(stats.last_activity,fav.last_activity,imp.last_activity) DESC NULLS LAST,mc.updated_at DESC`,
  NEW:`mc.updated_at DESC`,
  TITLE:`lower(COALESCE(mc.payload->>'title','')) ASC`,
  FAVOURITES:`COALESCE(fav.favorite_count,0) DESC,COALESCE(stats.list_count,0) DESC,mc.updated_at DESC`,
  MATCH:`(COALESCE(stats.list_count,0)+COALESCE(fav.favorite_count,0)*3+COALESCE(imp.impression_count,0)*2) DESC,mc.updated_at DESC`,
};
...
async function cachedCatalog(opts,page,perPage,status){
  ...
  const order=CATALOG_FALLBACK_SORT[requestedSort(opts.sort)]||CATALOG_FALLBACK_SORT.POPULAR;
...
const SORT_MAP={POPULAR:['START_DATE_DESC'],SCORE:['START_DATE_DESC'],TRENDING:['START_DATE_DESC'],NEW:['START_DATE_DESC'],TITLE:['TITLE_ROMAJI'],FAVOURITES:['START_DATE_DESC'],MATCH:['SEARCH_MATCH']};
...
export async function getCatalog(opts={}) {
  ...
  const sort=requestedSort(opts.sort),metricSort=['POPULAR','SCORE','TRENDING','FAVOURITES'].includes(sort);
  const key='catalog:v3:'+JSON.stringify({...opts,page,perPage,status,sort});
  return cacheRemember(key, opts.search?60:300, async()=>{
    ...
    try{
      ...
      const data=await gql(query,{page,perPage,search:opts.search?String(opts.search).slice(0,120):null,genre:opts.genre?String(opts.genre).slice(0,80):null,format:opts.format||null,season:opts.season||null,year:opts.year?Number(opts.year):null,status,sort:SORT_MAP[sort]||SORT_MAP.POPULAR});
      const result={pageInfo:data.Page.pageInfo,items:sortByInternalMetrics(await annotateMedia(data.Page.media.map(normalizeMedia),'ANIME'),sort)};
...
export async function getReading(opts={}) {
  ...
  const sort=requestedSort(opts.sort),metricSort=['POPULAR','SCORE','TRENDING','FAVOURITES'].includes(sort);
  const key='reading:v2:'+JSON.stringify({...opts,page,perPage,format,sort});
  return cacheRemember(key,opts.search?90:600,async()=>{
    ...
    try{
      ...
      const data=await gql(query,{page,perPage,search:opts.search?String(opts.search).slice(0,120):null,format,sort:SORT_MAP[sort]||SORT_MAP.POPULAR});
