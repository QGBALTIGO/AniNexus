...
function signature(a){return sha(JSON.stringify({title:a.title,summary:a.summary,image:a.imageUrl,facts:a.facts,sections:a.contentSections,sources:a.sources,coverage:a.coverage}),32)}
function body(a){return JSON.stringify({version:5,title:a.title,originalTitle:a.originalTitle||null,lead:a.summary,category:a.category,sourceLanguage:'pt-BR',imageUrl:a.imageUrl||'',facts:a.facts||[],sections:a.contentSections||[],sources:a.sources||[],readingMinutes:a.readingMinutes||2,wordCount:a.wordCount||0,coverage:a.coverage||null,updatedAt:a.updatedAt||a.publishedAt||null})}
...
async function persist(a){
  const sig=signature(a),published=new Date(a.publishedAt||Date.now()),expires=articleExpiry(published,RETENTION_DAYS),sources=mergeSources([],a.sources||[]),facts=splitFacts(a.facts||[],22),sections=Array.isArray(a.contentSections)?a.contentSections:[];
  ...
  if(row){
    ...
    await q(`UPDATE news_articles SET event_type=$2,title=$3,summary=$4,body=$5,status='published',image_url=COALESCE($6,image_url),image_alt=$7,source_name=$8,source_url=$9,external_url=$9,source_published_at=$10,expires_at=$11,language='pt-BR',translation_status='ready',facts=$12::jsonb,original_title=$13,source_author=$14,source_language='pt-BR',sources=$15::jsonb,source_count=$16,content_sections=$17::jsonb,reading_minutes=$18,word_count=$19,quality_score=GREATEST(quality_score,$20),content_signature=$21,content_version=$22,image_source_url=COALESCE($23,image_source_url),last_source_update_at=$24,last_seen_at=now(),updated_at=now() WHERE id=$1`,[
      row.id,a.eventType,a.title,a.summary,body(a),safeUrl(a.imageUrl)||null,a.title,a.sourceName,a.sourceUrl,published,expires,JSON.stringify(facts),cleanText(a.originalTitle||'',260)||null,null,JSON.stringify(sources),sources.length,JSON.stringify(sections),a.readingMinutes||2,a.wordCount||0,a.qualityScore||0,sig,version,a.imageUrl?a.sourceUrl:null,new Date(a.updatedAt||a.publishedAt||Date.now())
    ]);
  ...
  await q(`INSERT INTO news_articles(slug,source_kind,event_type,title,summary,body,spoiler,status,media_ids,external_url,published_at,image_url,image_alt,source_name,source_url,source_published_at,expires_at,language,translation_status,facts,source_hash,story_hash,original_title,source_author,source_language,sources,source_count,content_sections,reading_minutes,word_count,quality_score,content_signature,content_version,image_source_url,first_seen_at,last_seen_at,last_source_update_at)
    ...
    ON CONFLICT(source_hash) WHERE source_kind='AUTOMATED' AND source_hash IS NOT NULL DO UPDATE SET last_seen_at=now(),expires_at=EXCLUDED.expires_at,updated_at=now()`,[
      a.slug,a.eventType,a.title,a.summary,body(a),a.sourceUrl,published,safeUrl(a.imageUrl)||null,a.sourceName,expires,JSON.stringify(facts),a.sourceHash,a.storyHash,cleanText(a.originalTitle||'',260)||null,JSON.stringify(sources),sources.length,JSON.stringify(sections),a.readingMinutes||2,a.wordCount||0,a.qualityScore||0,sig,a.imageUrl?a.sourceUrl:null,new Date(a.updatedAt||a.publishedAt||Date.now())
    ]);
...
async function cycle(){
  ...
  try{
    ...
    for(const a of result.stories){try{const saved=await persist(a);if(saved.created)published++;if(saved.updated)updated++}catch(err){console.error('[news-v36-persist]',a.slug,err?.message||err)}}
    ...
    if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status=$2,sources_ok=$3,sources_failed=$4,collected_items=$5,grouped_stories=$6,published_stories=$7,skipped_translation=$8,skipped_quality=$9,duration_ms=$10,details=$11::jsonb WHERE id=$1`,[runId,status,ok,failed,result.collected,result.grouped,published,result.skippedTranslation,result.skippedQuality,duration,JSON.stringify({collectorVersion:36,updated,stories:result.stories.length,sourceResults:result.sourceResults})]).catch(()=>{});
    console.log('[news-worker-v36]',{status,ok,failed,collected:result.collected,stories:result.stories.length,published,updated,skippedTranslation:result.skippedTranslation,skippedQuality:result.skippedQuality,ms:duration});
  }catch(err){console.error('[news-worker-v36]',err?.stack||err?.message||err);if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status='failed',duration_ms=$2,details=$3::jsonb WHERE id=$1`,[runId,Date.now()-started,JSON.stringify({error:cleanText(err?.message||err,1200)})]).catch(()=>{})}finally{running=false}
}
