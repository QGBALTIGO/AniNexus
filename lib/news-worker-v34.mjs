...
import { collectEditorialStories } from './news-sources-v34.mjs';
...
const MAX_PER_SOURCE=Math.max(5,Math.min(30,Number(process.env.NEWS_MAX_PER_SOURCE||16)));
const MAX_AGE=Math.max(24,Math.min(168,Number(process.env.NEWS_MAX_SOURCE_AGE_HOURS||120)));
let running=false;

function signature(a){return sha(JSON.stringify({title:a.title,summary:a.summary,image:a.imageUrl,facts:a.facts,sections:a.contentSections,sources:a.sources}),32)}
function body(a){return JSON.stringify({version:4,title:a.title,originalTitle:a.originalTitle||null,lead:a.summary,category:a.category,sourceLanguage:'pt-BR',imageUrl:a.imageUrl||'',facts:a.facts||[],sections:a.contentSections||[],sources:a.sources||[],readingMinutes:a.readingMinutes||1,wordCount:a.wordCount||0})}
...
async function persist(a){
  const sig=signature(a),published=new Date(a.publishedAt||Date.now()),expires=articleExpiry(published,RETENTION_DAYS),sources=mergeSources([],a.sources||[]),facts=splitFacts(a.facts||[],8),sections=Array.isArray(a.contentSections)?a.contentSections:[];
  const existing=await q(`SELECT id,slug,title,summary,facts,content_sections,sources,image_url,quality_score,content_signature,content_version,source_hash
    FROM news_articles WHERE source_kind='AUTOMATED' AND story_hash=$1
      AND COALESCE(last_seen_at,source_published_at,published_at,created_at)>now()-interval '10 days'
  ...
  await q(`INSERT INTO news_articles(slug,source_kind,event_type,title,summary,body,spoiler,status,media_ids,external_url,published_at,image_url,image_alt,source_name,source_url,source_published_at,expires_at,language,translation_status,facts,source_hash,story_hash,original_title,source_author,source_language,sources,source_count,content_sections,reading_minutes,word_count,quality_score,content_signature,content_version,image_source_url,first_seen_at,last_seen_at,last_source_update_at)
    VALUES($1,'AUTOMATED',$2,$3,$4,$5,false,'published','[]'::jsonb,$6,$7,$8,$3,$9,$6,$7,$10,'pt-BR','ready',$11::jsonb,$12,$13,$14,null,'pt-BR',$15::jsonb,$16,$17::jsonb,$18,$19,$20,$21,1,$22,now(),now(),$7)
    ON CONFLICT(source_hash) WHERE source_kind='AUTOMATED' AND source_hash IS NOT NULL DO UPDATE SET last_seen_at=now(),expires_at=EXCLUDED.expires_at,updated_at=now()`,[
      a.slug,a.eventType,a.title,a.summary,body(a),a.sourceUrl,published,safeUrl(a.imageUrl)||null,a.sourceName,expires,JSON.stringify(facts),a.sourceHash,a.storyHash,cleanText(a.originalTitle||'',260)||null,JSON.stringify(sources),sources.length,JSON.stringify(sections),a.readingMinutes||1,a.wordCount||0,a.qualityScore||0,sig,a.imageUrl?a.sourceUrl:null
    ]);
...
async function cleanup(){
  ...
  await q(`DELETE FROM news_ingest_runs WHERE started_at<now()-interval '45 days'`).catch(()=>{});
}
...
async function cycle(){
  ...
  try{
    ...
    const result=await collectEditorialStories({maxPerSource:MAX_PER_SOURCE,maxAgeHours:MAX_AGE,maxStories:40,minQuality:MIN_QUALITY});
    let published=0,updated=0;
    for(const a of result.stories){try{const saved=await persist(a);if(saved.created)published++;if(saved.updated)updated++}catch(err){console.error('[news-v34-persist]',a.slug,err?.message||err)}}
    ...
    if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status=$2,sources_ok=$3,sources_failed=$4,collected_items=$5,grouped_stories=$6,published_stories=$7,skipped_translation=$8,skipped_quality=$9,duration_ms=$10,details=$11::jsonb WHERE id=$1`,[runId,status,ok,failed,result.collected,result.grouped,published,result.skippedTranslation,result.skippedQuality,duration,JSON.stringify({updated,sourceResults:result.sourceResults})]).catch(()=>{});
    console.log('[news-worker-v34]',{status,ok,failed,collected:result.collected,stories:result.stories.length,published,updated,skippedTranslation:result.skippedTranslation,skippedQuality:result.skippedQuality,ms:duration});
  }catch(err){console.error('[news-worker-v34]',err?.stack||err?.message||err);if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status='failed',duration_ms=$2,details=$3::jsonb WHERE id=$1`,[runId,Date.now()-started,JSON.stringify({error:cleanText(err?.message||err,1200)})]).catch(()=>{})}finally{running=false}
}
