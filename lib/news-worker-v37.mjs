...
async function cleanup({archive=true}={}){
...
async function cycle(){
  ...
  try{
    ...
    for(const a of result.stories){try{const saved=await persist(a);if(saved.created)published++;if(saved.updated)updated++;if(a.mediaGallery?.length||a.imageUrl)withMedia++}catch(err){console.error('[news-v39-persist]',a.slug,err?.message||err)}}
    await health(result);
    const ok=(result.sourceResults||[]).filter(x=>x.ok).length,failed=(result.sourceResults||[]).length-ok,hasHealthyContent=ok>0&&result.stories.length>0;
    await cleanup({archive:hasHealthyContent});
    const status=ok===0?'failed':!hasHealthyContent||failed?'partial':'success',duration=Date.now()-started;
    if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status=$2,sources_ok=$3,sources_failed=$4,collected_items=$5,grouped_stories=$6,published_stories=$7,skipped_translation=$8,skipped_quality=$9,duration_ms=$10,details=$11::jsonb WHERE id=$1`,[runId,status,ok,failed,result.collected,result.grouped,published,result.skippedTranslation,result.skippedQuality,duration,JSON.stringify({updated,withMedia,collectorVersion:39,archiveExpired:hasHealthyContent,sourceResults:result.sourceResults})]).catch(()=>{});
    console.log('[news-worker-v39]',{status,ok,failed,collected:result.collected,stories:result.stories.length,published,updated,withMedia,archiveExpired:hasHealthyContent,skippedTranslation:result.skippedTranslation,skippedQuality:result.skippedQuality,ms:duration});
  }catch(err){console.error('[news-worker-v39]',err?.stack||err?.message||err);if(runId)await q(`UPDATE news_ingest_runs SET finished_at=now(),status='failed',duration_ms=$2,details=$3::jsonb WHERE id=$1`,[runId,Date.now()-started,JSON.stringify({error:cleanText(err?.message||err,1200)})]).catch(()=>{})}finally{running=false}
}
