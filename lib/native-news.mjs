...
export async function getNativeNews({limit=20,offset=0,type=null,search=null}={}){
  ...
  const {rows}=await q(`SELECT ${FEED_FIELDS} FROM news_articles
    ...
    ORDER BY CASE WHEN image_url IS NULL OR image_url='' THEN 0 ELSE 1 END DESC,
      quality_score DESC,COALESCE(last_source_update_at,source_published_at,published_at) DESC NULLS LAST,created_at DESC
    LIMIT $3 OFFSET $4`,[safeType,safeSearch,safeLimit,safeOffset]);
  return rows;
}
...
export async function getNativeArticle(slug,{incrementView=true}={}){
  ...
  const {rows}=await q(`SELECT ${ARTICLE_FIELDS} FROM news_articles
    WHERE slug=$1 AND status='published' AND translation_status='ready' AND language='pt-BR'
      AND (expires_at IS NULL OR expires_at>now()) LIMIT 1`,[safeSlug]);
...
export async function getRelatedNews(article,{limit=4}={}){
  ...
  const {rows}=await q(`SELECT ${FEED_FIELDS} FROM news_articles
    WHERE id<>$1 AND status='published' AND translation_status='ready' AND language='pt-BR'
      AND (expires_at IS NULL OR expires_at>now()) AND event_type=$2
    ORDER BY CASE WHEN image_url IS NULL OR image_url='' THEN 0 ELSE 1 END DESC,
      quality_score DESC,COALESCE(last_source_update_at,source_published_at,published_at) DESC NULLS LAST LIMIT $3`,[article.id,article.event_type||'OTHER',safeLimit]);
  return rows;
}
