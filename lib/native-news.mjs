...
export async function getNativeNews({limit=20,offset=0,type=null,search=null}={}){
  ...
  const where=`status='published' AND (expires_at IS NULL OR expires_at>now())
    AND ($1::text IS NULL OR event_type=$1)
    AND ($2::text IS NULL OR title ILIKE '%'||$2||'%' OR summary ILIKE '%'||$2||'%' OR source_name ILIKE '%'||$2||'%')`;
  const [{rows},{rows:totalRows},{rows:facetRows},{rows:stampRows}]=await Promise.all([
    q(`SELECT ${FEED_FIELDS} FROM news_articles WHERE ${where}
    ...
    q(`SELECT count(*)::int total FROM news_articles WHERE ${where}`,[safeType,safeSearch]),
    q(`SELECT COALESCE(event_type,'OTHER') event_type,count(*)::int count FROM news_articles
      WHERE status='published' AND (expires_at IS NULL OR expires_at>now()) GROUP BY COALESCE(event_type,'OTHER') ORDER BY count DESC`),
    q(`SELECT max(COALESCE(last_seen_at,updated_at,published_at,created_at)) updated_at FROM news_articles
      WHERE status='published' AND (expires_at IS NULL OR expires_at>now())`)
  ]);
  return{
    items:rows,
    total:totalRows[0]?.total||0,
    facets:Object.fromEntries(facetRows.map(x=>[x.event_type,x.count])),
    updatedAt:stampRows[0]?.updated_at||null,
    limit:safeLimit,
    offset:safeOffset
  };
}

export async function getNativeArticle(slug,{incrementView=false}={}){
  await expireOldNews().catch(()=>{});
  ...
  if(item&&incrementView){
    q(`UPDATE news_articles SET view_count=view_count+1 WHERE id=$1`,[item.id]).catch(()=>{});
...
export async function getRelatedNews(article,{limit=4}={}){
  if(!article)return[];
  const media=Array.isArray(article.media_ids)?article.media_ids:[];
  ...
  const {rows}=await q(`SELECT ${FEED_FIELDS} FROM news_articles
    WHERE id<>$1 AND status='published' AND (expires_at IS NULL OR expires_at>now())
      AND (event_type=$2 OR (jsonb_array_length($3::jsonb)>0 AND media_ids ?| ARRAY(SELECT jsonb_array_elements_text($3::jsonb))))
    ORDER BY quality_score DESC,COALESCE(source_published_at,published_at) DESC NULLS LAST LIMIT $4`,[article.id,article.event_type||'OTHER',JSON.stringify(media),safeLimit]);
  return rows;
}
