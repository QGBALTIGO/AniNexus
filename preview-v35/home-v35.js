...
(() => {
  ...
  async function paintCharacters(){
    ...
    const draw=(focusId=null,animate=false)=>{
      ...
      if(focusId)requestAnimationFrame(()=>{const button=root.querySelector(`[data-character-favorite="${focusId}"]`);button?.focus({preventScroll:true});button?.closest('.nx47-character-card')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'nearest',inline:'center'});if(animate&&!matchMedia('(prefers-reduced-motion: reduce)').matches){button?.classList.remove('nx39-pop');void button?.offsetWidth;button?.classList.add('nx39-pop')}});
    };
