...
(() => {
  ...
  function awardFeature(item){const showWork=item.work&&String(item.work).toLowerCase()!==String(item.winner).toLowerCase();return `<article class="nx46-home-award-feature" data-home-award-feature-id="${esc(item.id)}"><div class="nx46-home-award-feature-art">${awardArt(item,true)}<span class="nx46-home-award-feature-shade"></span></div><div class="nx46-home-award-feature-copy">${awardMark()}<p class="nx46-home-award-category">${esc(item.category)}</p><h3>${esc(item.winner)}</h3>${showWork?`<p class="nx46-home-award-work">${esc(item.work)}</p>`:''}${item.credit?`<p class="nx46-home-award-credit">${esc(item.credit)}</p>`:''}</div></article>`}
  ...
  function paintAchievements(){const root=app.querySelector('#nx35Achievements');if(!root)return;const items=achievementCards(),section=root.closest('.nx35-achievement-section');if(section)section.hidden=!items.length;if(!items.length){root.replaceChildren();return}root.innerHTML=items.map(([a,b,icon])=>`<div class="nx35-achievement"><span>${icon}</span><div><strong>${esc(a)}</strong><p>${esc(b)}</p></div></div>`).join('')}
  ...
  async function paintAwards(){
    ...
    const drawFeature=(animate=true)=>{const template=document.createElement('template');template.innerHTML=awardFeature(selected);const next=template.content.firstElementChild;if(!next)return;bindFallback(next);if(!activeFeature||!activeFeature.isConnected||!animate||reducedMotion){clearTransition();feature.replaceChildren(next);next.classList.add('is-current');activeFeature=next;bindNavigation();preloadNext();return}clearTransition();feature.querySelectorAll('.nx46-home-award-feature').forEach(node=>{if(node!==activeFeature)node.remove()});const previous=activeFeature;next.classList.add('is-entering','is-current');feature.append(next);next.getBoundingClientRect();previous.classList.remove('is-current');previous.classList.add('is-leaving');next.classList.remove('is-entering');activeFeature=next;bindNavigation();transitionTimer=setTimeout(()=>{previous.remove();timers.delete(transitionTimer);transitionTimer=null},680);timers.add(transitionTimer);preloadNext()};
