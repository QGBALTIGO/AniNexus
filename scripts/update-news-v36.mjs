...
try{result=await collectEditorialStories({maxPerSource:Number(process.env.NEWS_MAX_PER_SOURCE||45),maxAgeHours:Number(process.env.NEWS_MAX_SOURCE_AGE_HOURS||168),maxStories:Number(process.env.NEWS_MAX_STORIES||60),minQuality:MIN_QUALITY})}catch(err){console.error('[news-static-v37] coleta falhou; preservando feed anterior:',err?.message||err)}
const fresh=(result.stories||[]).map(staticItem).filter(valid),map=new Map();
for(const x of [...fresh,...previousItems]){const key=x.slug||x.id,prev=map.get(key);if(!prev||Number(x.qualityScore||0)>Number(prev.qualityScore||0)||Number(x.wordCount||0)>Number(prev.wordCount||0)||Number(x.coverage?.mediaCount||0)>Number(prev.coverage?.mediaCount||0)||new Date(x.updatedAt||x.publishedAt)>new Date(prev.updatedAt||prev.publishedAt))map.set(key,x)}
...
if(items.length<MIN_SAFE_FEED&&previousItems.length>=items.length){const fallback=new Map();for(const x of [...items,...previousItems])fallback.set(x.slug||x.id,x);items=[...fallback.values()].filter(valid).sort((a,b)=>new Date(b.publishedAt)-new Date(a.publishedAt)).slice(0,MAX_ITEMS);console.warn(`[news-static-v37] somente ${fresh.length} matérias novas válidas; preservando ${items.length} itens para evitar feed raso/vazio`)}
if(!items.length)throw new Error('Proteção do feed V37: nenhuma matéria PT-BR válida; o arquivo anterior não será substituído.');
...
const doc={generatedAt:new Date().toISOString(),retentionDays:RETENTION_DAYS,language:'pt-BR',editorialMode:'internal-deep-media',collectorVersion:37,sources:['JBox WordPress/Anime/Mangá','Crunchyroll Notícias PT-BR','MyAnimeList (complementar)','Anime News Network (complementar)'],sourceHealth:result.sourceResults,stats,items};
await fs.mkdir(path.dirname(OUT),{recursive:true});await fs.writeFile(OUT,JSON.stringify(doc,null,2)+'\n');
console.log(`[news-static-v37] ${items.length} matérias; ${stats.deepArticles} profundas; ${stats.withGallery} com galeria; ${stats.withEmbeds} com vídeo; média ${stats.avgWords} palavras`);
