...
test('Home schedule uses bare official brands aligned episodes and one content surface',async({page})=>{
  ...
  const scheduleTypography=await schedule.locator('.nx18-card').first().evaluate(card=>{const weight=selector=>parseInt(getComputedStyle(card.querySelector(selector)).fontWeight,10);return{air:weight('.nx18-air'),time:weight('.nx18-air b'),title:weight('h3'),countdown:weight('.nx18-countdown>span'),countdownLabel:weight('.nx18-countdown small'),score:weight('.nx18-score b'),episodeLabel:weight('.nx18-episode small'),titleSpacing:getComputedStyle(card.querySelector('h3')).letterSpacing}});
  expect(scheduleTypography).toEqual({air:600,time:600,title:600,countdown:600,countdownLabel:500,score:600,episodeLabel:600,titleSpacing:'normal'});
