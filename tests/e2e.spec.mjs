...
const pageUrl=route=>`${ORIGIN}?build=44.38.1&p=${encodeURIComponent(route)}`;
...
test('detail metrics and tags are presented in the requested Portuguese AniQuim pattern',async({page})=>{
  ...
  await expect(page.locator('[data-nx22-rating],[data-nx22-rating-star]')).toHaveCount(0);await expect(page.locator('.nx22-hero-bg')).toHaveClass(/is-invalid/);
  ...
  await expect(page.locator('.nx22-hero .nx22-airing')).toHaveCount(0);await expect(page.locator('.nx22-content>.nx22-overview-schedule')).toBeVisible();await expect(page.locator('#nx22Synopsis')).toHaveText('Uma história.');
  const schedule=await page.locator('.nx22-overview-schedule .nx22-airing').boundingBox(),content=await page.locator('.nx22-content').boundingBox(),synopsis=await page.getByRole('heading',{name:'Sinopse'}).boundingBox();expect(schedule.height).toBeGreaterThanOrEqual(60);expect(Math.abs((schedule.x+schedule.width/2)-(content.x+content.width/2))).toBeLessThanOrEqual(1);expect(schedule.y+schedule.height).toBeLessThan(synopsis.y);
});

test('anime and manga synopsis appears immediately and upgrades to validated Portuguese in the background',async({page})=>{
  ...
  await page.route('https://translate.googleapis.com/**',async route=>{await new Promise(resolve=>setTimeout(resolve,700));await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify([[[translated,'source']]])})});
  await page.route('https://api.mymemory.translated.net/**',route=>route.abort('failed'));
  for(const item of [{type:'anime',id:101,mediaType:'ANIME',source:'A young traveler begins a decisive adventure with close friends, faces dangerous enemies, and learns that their own story can change the entire world.'},{type:'manga',id:202,mediaType:'MANGA',source:'After moving to a distant town, a quiet student meets an unlikely companion and uncovers a mystery that connects their families across generations.'}]){
    ...
    await expect(page.locator('#nx22Synopsis')).toHaveText(item.source,{timeout:500});await expect(page.locator('#nx22SynopsisNote')).toHaveText('Traduzindo…');await expect(page.locator('#nx22Synopsis')).toHaveText(translated,{timeout:3000});await expect(page.locator('#nx22SynopsisNote')).toHaveText('Em português');
  }
});
...
test('anime and manga details use AniQuim-sized controls, stable clicks and filtered genres',async({page})=>{
  ...
  for(const item of [{route:'/anime/anime-teste-101',id:101,key:'aninexus:mediaState:v2',catalog:'/animes/catalogo',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:anime-catalog-state'},{route:'/manga/manga-teste-202',id:202,key:'aninexus:mangaState:v2',catalog:'/mangas',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:reading-catalog-state'}]){
    ...
    await page.locator('[data-nx22-tab="elenco"]').click();expect(await page.evaluate(()=>Boolean(window.__nxStableDocument))).toBe(true);
