...
const pageUrl=route=>`${ORIGIN}?build=44.47.1&p=${encodeURIComponent(route)}`;
...
test('detail tabs stay in one document, load real sections and preserve cross-media franchise links',async({page})=>{
  ...
  await page.route('**/api/me/character-favorites/*',route=>{const request=route.request();characterWrite={method:request.method(),body:request.postDataJSON()};return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,characterId:Number(new URL(request.url()).pathname.split('/').pop()),favorite:true,favoriteCount:4})})});
  await page.setViewportSize({width:1280,height:800});await page.goto(firstVisitUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await allowAccountActions(page,{id:'detail-character-member'});await expect(page.locator('.nx22-detail:not(.nx22-fail)')).toBeVisible({timeout:30000});await expect(page.locator('.nx22-headcopy h1')).toHaveText('Anime Abas 101');await page.evaluate(()=>window.__nxTabDocument={created:Date.now()});
  await expect(page.locator('.nx42-impressions')).toHaveCount(1);await expect(page.locator('.nx42-impression-form')).toHaveCount(0);await expect(page.locator('[data-nx42-impression-login]')).toBeVisible();await page.evaluate(()=>dispatchEvent(new CustomEvent('aninexus:account-identity-changed',{detail:{user:{id:'detail-member'}}})));await expect(page.locator('.nx42-impression-form')).toBeVisible();await expect(page.locator('.nx22-translation-note')).toHaveCount(0);await expect(page.getByRole('tab',{name:'Atividade'})).toHaveCount(0);await expect(page.getByRole('tab',{name:'Impressões'})).toHaveCount(0);
  ...
  await expect(page.locator('#nx22Activity')).toHaveAttribute('data-state','ready');await expect(page.locator('#nx22Activity')).toContainText('Quero ver');await expect(page.locator('#nx22Activity')).toContainText('Amei 60%');await expect(page.locator('#nx22Activity')).toContainText('12 membros acompanham');
  const communityOrder=await page.locator('#nx22Panel').evaluate(panel=>{const activity=panel.querySelector('#nx22Activity'),impressions=panel.querySelector('#nx22Impressions');return Boolean(activity&&impressions&&(activity.compareDocumentPosition(impressions)&Node.DOCUMENT_POSITION_FOLLOWING))});expect(communityOrder).toBe(true);
  const communityGap=await page.evaluate(()=>{const activity=document.querySelector('.nx22-activity-card').getBoundingClientRect(),impressions=document.querySelector('#nx22Impressions').getBoundingClientRect();return impressions.top-activity.bottom});expect(communityGap).toBeGreaterThanOrEqual(24);
  const neutralUrl=page.url();await page.evaluate(()=>{window.__nxDetailArticle=document.querySelector('.nx22-detail');window.__nxDetailPanel=document.querySelector('#nx22Panel')});await page.locator('.nx22-synopsis').click({position:{x:8,y:8}});await page.waitForTimeout(350);expect(page.url()).toBe(neutralUrl);expect(await page.evaluate(()=>window.__nxDetailArticle===document.querySelector('.nx22-detail')&&window.__nxDetailPanel===document.querySelector('#nx22Panel'))).toBe(true);
  await page.getByRole('tab',{name:'Personagens & equipe'}).click();await expect(page.locator('#nx22Panel')).toContainText('Personagem Principal');await expect(page.locator('#nx22Panel')).toContainText('Diretora Teste');await expect(page.locator('#nx22Panel')).toContainText('Direção');await expect(page.locator('#nx22Panel')).not.toContainText('Licenciadora indevida');await expect(page.locator('.nx42-impressions')).toHaveCount(0);const castRail=page.locator('.nx22-cast-panel [data-nx22-rail]').first(),castNext=page.locator('.nx22-cast-panel [data-nx44-rail-dir="next"]').first();await expect(castRail).toHaveClass(/nx44-rail/);await expect(castNext).toBeEnabled();await castNext.click();await expect.poll(()=>castRail.evaluate(element=>element.scrollLeft)).toBeGreaterThan(100);await expect(page.locator('[data-nx22-character-favorite="1"]')).toHaveAttribute('aria-pressed','true');await page.locator('[data-nx22-character-favorite="10"]').click();await expect.poll(()=>characterWrite).toMatchObject({method:'PUT',body:{name:'Personagem 2',work:'Anime Abas 101',mediaId:101,mediaType:'ANIME'}});await expect(page.locator('[data-nx22-character-favorite="10"]')).toHaveAttribute('aria-pressed','true');
  await page.getByRole('tab',{name:'Franquia'}).click();await expect(page.locator('.nx22-related')).toHaveCount(7);await expect(page.locator('.nx22-related').first()).toContainText('Mangá Relacionado');await expect(page.locator('.nx22-related').first()).toHaveAttribute('data-nx22-media-type','MANGA');await expect(page.locator('[data-nx22-search-title="Anime Relacionado por MAL"]')).toBeVisible();await expect(page.locator('.nx42-impressions')).toHaveCount(0);const relationRail=page.locator('.nx22-related-grid'),relationNext=page.locator('.nx22-detail-rail-section [data-nx44-rail-dir="next"]');await expect(relationRail).toHaveClass(/nx44-rail/);await expect(relationNext).toBeEnabled();await relationNext.click();await expect.poll(()=>relationRail.evaluate(element=>element.scrollLeft)).toBeGreaterThan(100);
  await page.getByRole('tab',{name:'Recomendações'}).click();await expect(page.locator('.nx22-rec')).toContainText('Anime Recomendado');
  await page.getByRole('tab',{name:'Geral',exact:true}).click();await expect(page.locator('.nx42-impressions')).toHaveCount(1);
...
for(const item of [{kind:'anime',route:'/anime/anime-teste-101',id:101,catalog:'/animes/catalogo',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:anime-catalog-state'},{kind:'mangá',route:'/manga/manga-teste-202',id:202,catalog:'/mangas',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:reading-catalog-state'}]){
  test(`${item.kind} detail uses AniQuim-sized controls, stable clicks and filtered genres`,async({page})=>{
    ...
    await expect(page.locator('[data-nx22-back]')).toBeVisible();await expect(page.locator('[data-nx22-rating],[data-nx22-rating-star]')).toHaveCount(0);
...
test('visitors reach login before publishing impressions or news comments',async({page})=>{
  ...
  await page.goto(pageUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx42-impressions')).toBeVisible({timeout:30000});await expect(page.locator('.nx42-impression-form')).toHaveCount(0);await page.getByRole('button',{name:'Entre na sua conta para compartilhar uma impressão'}).click();await waitForLogin(page);expect(writes).toHaveLength(0);
...
test('anime detail light theme styles navigation and community access coherently',async({page})=>{await page.addInitScript(()=>localStorage.setItem('aninexus:theme','light'));await page.goto(pageUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx22-detail:not(.nx22-fail)')).toBeVisible({timeout:30000});await expect(page.locator('html')).toHaveAttribute('data-theme','light');await expect(page.locator('.topbar')).toHaveCSS('background-color','rgba(0, 0, 0, 0)');await page.evaluate(()=>scrollTo(0,160));await expect(page.locator('.topbar')).toHaveClass(/is-scrolled/);await expect(page.locator('.topbar')).not.toHaveCSS('background-color','rgba(0, 0, 0, 0)');await expect(page.locator('.nx22-info-card').first()).toHaveCSS('background-color','rgb(255, 255, 255)');await expect(page.locator('[data-nx42-impression-login]')).toHaveCSS('color','rgb(107, 52, 69)');await noOverflow(page,2)});
