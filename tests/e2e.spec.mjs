...
const pageUrl=route=>`${ORIGIN}?build=44.35.3&p=${encodeURIComponent(route)}`;
...
test('anime and manga details keep compact controls, fill ratings and open filtered genres',async({page})=>{
  ...
  for(const item of [{route:'/anime/anime-teste-101',id:101,key:'aninexus:mediaState:v2',catalog:'/animes/catalogo',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:anime-catalog-state'},{route:'/manga/manga-teste-202',id:202,key:'aninexus:mangaState:v2',catalog:'/mangas',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:reading-catalog-state'}]){
    ...
    expect(parseFloat(await title.evaluate(element=>getComputedStyle(element).fontSize))).toBeLessThanOrEqual(30);expect(parseFloat(await list.evaluate(element=>getComputedStyle(element).height))).toBeLessThanOrEqual(40);expect(parseFloat(await favorite.evaluate(element=>getComputedStyle(element).width))).toBeLessThanOrEqual(40);await expect(stars).toHaveCount(5);
    await expect(page.locator('.nx22-stats').first()).toContainText('nota de 24 membros');
    await stars.nth(3).click({position:{x:18,y:11}});await expect(page.locator('[data-nx22-rating] output')).toHaveText('8.0');
