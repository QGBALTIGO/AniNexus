...
test('Programação opens one modal and saved status reaches Home and Community',async({page})=>{
  ...
  await page.goto(pageUrl('/'),{waitUntil:'domcontentloaded'});await page.evaluate(()=>{window.AniNexusAccountData=async()=>({username:'e2e-member',displayName:'Membro de teste'});dispatchEvent(new CustomEvent('aninexus:account-identity-changed'))});await expect(page.locator('.nx35-home')).toBeVisible({timeout:30000});await expect(page.locator('#nx35CommunityHero')).toContainText('está assistindo',{timeout:15000});if(animeTitle)await expect(page.locator('#nx35CommunityHero')).toContainText(animeTitle,{timeout:15000});
...
test('Source handoff links the authenticated AniNexus account and can be hidden or disconnected',async({page})=>{
  ...
  await page.route('**/runtime-config.js*',route=>route.fulfill({contentType:'application/javascript',body:`window.__ANINEXUS_CONFIG__={environment:'test',siteOrigin:location.origin,apiOrigin:'https://api.clerk.com',clerkPublishableKey:['pk','test','dGVzdC5jbGVyay5hY2NvdW50cy5kZXYk'].join('_'),authEnabled:true};`}));
  await page.route('https://test.clerk.accounts.dev/npm/@clerk/ui@1/dist/ui.browser.js',route=>route.fulfill({contentType:'application/javascript',body:'window.__internal_ClerkUICtor=function(){};'}));
  await page.route('https://test.clerk.accounts.dev/npm/@clerk/clerk-js@6/dist/clerk.browser.js',route=>route.fulfill({contentType:'application/javascript',body:`window.Clerk={user:{id:'clerk_source'},session:{getToken:async()=>"token"},load:async()=>{},addListener:()=>{},signOut:async()=>{}};`}));
  await page.route('https://api.clerk.com/**',async route=>{
...
test('public profile deep-links to an unlocked achievement and shows three pinned badges',async({page},testInfo)=>{
  ...
  for(const width of [390,768,1440]){
    await page.setViewportSize({width,height:900});
    const badges=await profile.locator('.nx48-profile-achievements .nx48-badge').evaluateAll(items=>items.map(item=>{const badge=item.getBoundingClientRect(),holder=item.querySelector('i').getBoundingClientRect(),icon=item.querySelector('svg').getBoundingClientRect();return{dx:Math.abs((badge.left+badge.right-icon.left-icon.right)/2),dy:Math.abs((badge.top+badge.bottom-icon.top-icon.bottom)/2),badgeWidth:badge.width,holderWidth:holder.width,iconWidth:icon.width}}));
    expect(badges.length).toBeGreaterThan(0);for(const badge of badges){expect(badge.dx).toBeLessThanOrEqual(1);expect(badge.dy).toBeLessThanOrEqual(1);expect(badge.iconWidth).toBe(badge.holderWidth);expect(badge.iconWidth).toBeLessThanOrEqual(22);expect(badge.iconWidth/badge.badgeWidth).toBeLessThan(.5)}
