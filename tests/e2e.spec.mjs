...
const pageUrl=route=>`${ORIGIN}?build=44.46.0&p=${encodeURIComponent(route)}`;
...
test('detail tabs stay in one document, load real sections and preserve cross-media franchise links',async({page})=>{
  ...
  await page.getByRole('tab',{name:'Franquia'}).click();await expect(page.locator('.nx22-related')).toHaveCount(2);await expect(page.locator('.nx22-related').first()).toContainText('Mangá Relacionado');await expect(page.locator('.nx22-related').first()).toHaveAttribute('data-nx22-media-type','MANGA');await expect(page.locator('[data-nx22-search-title="Anime Relacionado por MAL"]')).toBeVisible();await expect(page.locator('.nx42-impressions')).toHaveCount(0);
