...
const pageUrl=route=>`${ORIGIN}?build=44.50.0&p=${encodeURIComponent(route)}`;
...
test('detail tabs stay in one document, load real sections and preserve cross-media franchise links',async({page})=>{
  ...
  const characterWrites=[];let characterDeleteAttempts=0;
  await page.route('**/api/anime/101',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(media)}));
  await page.route(/\/api\/anime\/101\/impressions(?:\?.*)?$/,route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[]})}));
  ...
  await page.setViewportSize({width:1280,height:800});await page.goto(firstVisitUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await allowAccountActions(page,{id:'detail-character-member',username:'detail-member'});await expect(page.locator('.nx22-detail:not(.nx22-fail)')).toBeVisible({timeout:30000});await expect(page.locator('.nx22-headcopy h1')).toHaveText('Anime Abas 101');await page.evaluate(()=>window.__nxTabDocument={created:Date.now()});
  await expect(page.locator('.nx22-translation-note')).toHaveCount(0);await expect(page.getByRole('tab',{name:'Atividade'})).toHaveCount(0);await expect(page.getByRole('tab',{name:'Impressões'})).toBeVisible();await expect(page.getByRole('tab',{name:'Comentários'})).toBeVisible();
  ...
  await expect(page.locator('.nx50-social')).toHaveCount(0);await page.getByRole('tab',{name:'Impressões'}).click();await expect(page.locator('#nx22Panel .nx50-social')).toBeVisible();await expect(page.locator('#nx22Panel')).toContainText('Sua opinião geral sobre a obra');await expect(page.locator('.nx50-composer[data-nx50-composer="impression"]')).toBeVisible();await expect(page.locator('#nx22Activity')).toHaveCount(0);
  await page.getByRole('tab',{name:'Comentários'}).click();await expect(page.locator('#nx22Panel .nx50-comments')).toBeVisible();await expect(page.locator('#nx22Panel')).toContainText('Converse sobre um episódio específico');await expect(page.locator('[data-nx50-episode]')).toHaveValue('1');await expect(page.locator('.nx50-composer[data-nx50-composer="comment"]')).toBeVisible();await expect(page.locator('#nx22Activity')).toHaveCount(0);
  ...
  await page.getByRole('tab',{name:'Personagens & equipe'}).click();await expect(page.locator('#nx22Panel')).toContainText('Personagem Principal');await expect(page.locator('#nx22Panel')).toContainText('Diretora Teste');await expect(page.locator('#nx22Panel')).toContainText('Direção');await expect(page.locator('#nx22Panel')).not.toContainText('Licenciadora indevida');await expect(page.locator('.nx50-social')).toHaveCount(0);await expect(page.locator('.nx22-cast-panel [data-nx22-rail],.nx22-cast-panel [data-nx44-rail-dir]')).toHaveCount(0);await expect(page.locator('.nx22-cast-panel .nx22-people-grid')).toHaveCount(2);const castColumns=await page.locator('.nx22-cast-panel .nx22-people-grid').first().evaluate(grid=>[...grid.children].slice(0,4).map(item=>Math.round(item.getBoundingClientRect().left)));expect(new Set(castColumns.slice(0,3)).size).toBe(3);expect(castColumns[3]).toBe(castColumns[0]);await expect(page.locator('[data-nx22-character-favorite="1"]')).toHaveAttribute('aria-pressed','true');await page.locator('[data-nx22-character-favorite="10"]').click();await expect.poll(()=>characterWrites.at(-1)).toMatchObject({method:'PUT',body:{name:'Personagem 2',work:'Anime Abas 101',mediaId:101,mediaType:'ANIME'}});await expect(page.locator('[data-nx22-character-favorite="10"]')).toHaveAttribute('aria-pressed','true');await page.locator('[data-nx22-character-favorite="10"]').click();await expect.poll(()=>characterWrites.filter(item=>item.method==='DELETE').length).toBe(2);await expect(page.locator('[data-nx22-character-favorite="10"]')).toHaveAttribute('aria-pressed','false');await expect(page.locator('#toastRoot')).not.toContainText('Não foi possível atualizar o personagem agora');
...
test('episode comments protect progress and reveal only the selected spoiler segment',async({page})=>{
  const comment={id:'20000000-0000-4000-8000-000000000001',parent_id:null,root_id:'20000000-0000-4000-8000-000000000001',depth:0,body:'O começo é seguro, mas ||o final muda tudo||.',has_spoilers:true,hideSpoilers:true,created_at:new Date().toISOString(),likes_count:3,replies_count:0,user:{username:'membro',displayName:'Membro Teste',avatarUrl:pixel}};
  let commentReads=0;
  await page.addInitScript(()=>{const state={status:'CURRENT',progress:2,score:8,reaction:'',updatedAt:Date.now()};localStorage.setItem('aninexus:mediaState:v2',JSON.stringify({101:state}));localStorage.setItem('aninexus:mediaState:v1',JSON.stringify({101:state}))});
  await page.route(/\/api\/anime\/101\/episodes\/(\d+)\/comments\/spoiler-check(?:\?.*)?$/,route=>{const episode=Number(new URL(route.request().url()).pathname.split('/').at(-3));return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(episode<=2?{allowed:true,userProgress:2,episodesAhead:0,status:'CURRENT'}:{allowed:false,userProgress:2,episodesAhead:episode-2,status:'CURRENT'})})});
  await page.route(/\/api\/anime\/101\/episodes\/(\d+)\/comments(?:\?.*)?$/,route=>{commentReads++;const episode=Number(new URL(route.request().url()).pathname.split('/').at(-2));return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({animeId:101,episode,items:[comment],sort:'recent',hideSpoilers:true})})});
  await page.route('**/api/me/likes?**',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[]})}));
  await page.goto(pageUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx22-detail')).toBeVisible({timeout:30000});await allowAccountActions(page,{id:'progress-member',username:'progress-member'});await page.getByRole('tab',{name:'Comentários'}).click();await expect(page.locator('[data-nx50-episode]')).toHaveValue('2');await expect(page.locator('[data-nx50-comment]')).toBeVisible();expect(commentReads).toBe(1);
  const episodeInput=page.locator('[data-nx50-episode]');await episodeInput.fill('9');await episodeInput.blur();await expect(page.locator('.nx50-progress-warning')).toContainText('7 episódios');await expect(page.locator('[data-nx50-comment]')).toHaveCount(0);expect(commentReads).toBe(1);
  await page.getByRole('button',{name:'Ver mesmo assim'}).click();await expect(page.locator('[data-nx50-comment]')).toBeVisible();expect(commentReads).toBe(2);const spoiler=page.locator('[data-nx50-spoiler]');await expect(spoiler).toHaveAttribute('aria-label','Revelar trecho com spoiler');await expect(page.locator('.nx50-body')).toContainText('O começo é seguro');await spoiler.click();await expect(spoiler).toHaveClass(/revealed/);await noOverflow(page,2);
});
...
for(const item of [{kind:'anime',route:'/anime/anime-teste-101',id:101,catalog:'/animes/catalogo',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:anime-catalog-state'},{kind:'mangá',route:'/manga/manga-teste-202',id:202,catalog:'/mangas',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:reading-catalog-state'}]){
  test(`${item.kind} detail uses AniQuim-sized controls, stable clicks and filtered genres`,async({page})=>{
    ...
    await expect(page.locator('[data-nx22-back]')).toBeVisible();await expect(page.locator('[data-nx22-rating],[data-nx22-rating-star]')).toHaveCount(0);await expect(page.getByRole('tab',{name:'Impressões'})).toBeVisible();if(item.kind==='mangá')await expect(page.getByRole('tab',{name:'Comentários'})).toHaveCount(0);else await expect(page.getByRole('tab',{name:'Comentários'})).toBeVisible();
...
test('single Home impression is complete and does not look like a broken rail',async({page})=>{
  ...
  await expect(card).toBeVisible({timeout:30000});
  await expect(card).toContainText('Uma estreia excelente');await expect(page.locator('[data-home-imp-filter="all"]')).toHaveClass(/active/);await expect(page.locator('[data-home-imp-sort="recent"]')).toHaveClass(/active/);await expect(page.locator('[data-home-imp-hide]')).toBeChecked();const partialSpoiler=card.locator('[data-home-imp-spoiler]');await expect(partialSpoiler).toHaveAttribute('aria-label','Revelar trecho com spoiler');await partialSpoiler.click();await expect(partialSpoiler).toHaveClass(/revealed/);
  ...
  await expect(card.locator('.nx38-impression-meta-row')).toContainText('♡ 7');
  await expect(card.locator('.nx38-impression-meta-row')).toContainText('↩ 2');
...
test('visitors reach login before publishing impressions or news comments',async({page})=>{
  ...
  await page.route(/\/api\/anime\/101\/impressions(?:\?.*)?$/,route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[]})}));await page.goto(pageUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx22-detail')).toBeVisible({timeout:30000});await page.getByRole('tab',{name:'Impressões'}).click();await expect(page.locator('.nx50-login-cta')).toBeVisible();await expect(page.locator('.nx50-composer')).toHaveCount(0);await page.getByRole('button',{name:'Entre na sua conta para publicar uma impressão'}).click();await waitForLogin(page);expect(writes).toHaveLength(0);

  const slug='comentario-de-visitante-teste';await page.route(`**/api/news/${slug}`,route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({id:'visitor-news',slug,title:'Notícia de teste',summary:'Resumo de teste.',event_type:'ANIME',source_name:'AniNexus Notícias',language:'pt-BR',published_at:new Date().toISOString(),facts:[],body:{sections:[{heading:'Atualização',paragraphs:['Conteúdo de teste para comentários.']}]}})}));await page.goto(firstVisitUrl(`/noticias/${slug}`),{waitUntil:'domcontentloaded'});const comment=page.locator('.nx42-news-comment-form');await expect(comment).toBeVisible({timeout:30000});await comment.locator('textarea').fill('Um comentário de visitante');await comment.getByRole('button',{name:'Publicar comentário'}).click();await page.waitForURL(url=>{const current=new URL(url);return current.searchParams.get('p')==='/login'||current.pathname.replace(/\/+$/,'').endsWith('/login')},{timeout:15000});expect(writes).toHaveLength(0);
});
