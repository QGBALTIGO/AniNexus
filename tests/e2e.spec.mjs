...
test('mobile drawer prioritizes the account, library and compact destinations',async({page},testInfo)=>{
  ...
  const accountAlignment=await auth.evaluate(element=>{const avatar=element.querySelector('.drawer-account-avatar').getBoundingClientRect(),tools=element.querySelector('.drawer-account-tools').getBoundingClientRect(),box=element.getBoundingClientRect();return{centerDelta:Math.abs((avatar.top+avatar.height/2)-(tools.top+tools.height/2)),left:avatar.left-box.left,right:box.right-tools.right}});
  expect(accountAlignment.centerDelta).toBeLessThanOrEqual(2);
