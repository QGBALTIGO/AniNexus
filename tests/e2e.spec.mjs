...
const pageUrl=route=>`${ORIGIN}?build=44.41.0&p=${encodeURIComponent(route)}`;
...
test('detail tabs stay in one document, load real sections and preserve cross-media franchise links',async({page})=>{
  ...
  await expect(page.locator('.nx42-impressions')).toHaveCount(1);await expect(page.locator('.nx22-translation-note')).toHaveCount(0);await expect(page.getByRole('tab',{name:'Atividade'})).toHaveCount(0);
  ...
  await page.getByRole('tab',{name:'Impressões'}).click();await expect(page.locator('.nx42-impression-form')).toBeVisible();await expect(page.locator('.nx42-impressions')).toHaveCount(1);
...
test('mobile detail navigation opens as an AniQuim-style bottom sheet',async({page})=>{
  await page.setViewportSize({width:390,height:844});await page.goto(firstVisitUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx22-detail:not(.nx22-fail)')).toBeVisible({timeout:30000});const toggle=page.locator('[data-nx22-tabs-toggle]');await expect(toggle).toBeVisible();await expect(toggle).toContainText('Visão geral');await toggle.click();await expect(toggle).toHaveAttribute('aria-expanded','true');await expect(page.locator('.nx22-tabs-sheet')).toBeVisible();await page.getByRole('tab',{name:'Franquia'}).click();await expect(toggle).toContainText('Franquia');await expect(toggle).toHaveAttribute('aria-expanded','false');await expect(page.locator('.nx22-tabs-sheet')).toBeHidden();await noOverflow(page,2);
});
...
for(const item of [{kind:'anime',route:'/anime/anime-teste-101',id:101,catalog:'/animes/catalogo',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:anime-catalog-state'},{kind:'mangá',route:'/manga/manga-teste-202',id:202,catalog:'/mangas',filterSelector:'[data-nx22-genre]',filterKey:'genre',filterValue:'Action',catalogState:'nx:v46:reading-catalog-state'}]){
  test(`${item.kind} detail uses AniQuim-sized controls, stable clicks and filtered genres`,async({page})=>{
    ...
    await page.locator('[data-nx22-tabs-toggle]').click();await expect(page.locator('.nx22-tabs-sheet')).toBeVisible();await page.locator('[data-nx22-tab="elenco"]').click();expect(await page.evaluate(()=>Boolean(window.__nxStableDocument))).toBe(true);
...
test('visitors reach login before publishing impressions or news comments',async({page})=>{
  ...
  await page.goto(pageUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await page.getByRole('tab',{name:'Impressões'}).click();const impression=page.locator('.nx42-impression-form');await expect(impression).toBeVisible({timeout:30000});await impression.locator('textarea').fill('Uma impressão de visitante');await impression.getByRole('button',{name:'Publicar impressão'}).click();await waitForLogin(page);expect(writes).toHaveLength(0);
