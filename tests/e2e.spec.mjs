...
const pageUrl=route=>`${ORIGIN}?build=44.42.0&p=${encodeURIComponent(route)}`;
...
test('detail tabs stay in one document, load real sections and preserve cross-media franchise links',async({page})=>{
  ...
  await page.setViewportSize({width:1280,height:800});await page.goto(firstVisitUrl('/anime/anime-teste-101'),{waitUntil:'domcontentloaded'});await expect(page.locator('.nx22-detail:not(.nx22-fail)')).toBeVisible({timeout:30000});await expect(page.locator('.nx22-headcopy h1')).toHaveText('Anime Abas 101');await page.evaluate(()=>window.__nxTabDocument={created:Date.now()});
  ...
  await page.getByRole('tab',{name:'Personagens & equipe'}).click();await expect(page.locator('#nx22Panel')).toContainText('Personagem Principal');await expect(page.locator('#nx22Panel')).toContainText('Diretora Teste');await expect(page.locator('.nx42-impressions')).toHaveCount(0);
  await page.getByRole('tab',{name:'Franquia'}).click();await expect(page.locator('.nx22-related')).toContainText('Mangá Relacionado');await expect(page.locator('.nx22-related')).toHaveAttribute('data-nx22-media-type','MANGA');await expect(page.locator('.nx42-impressions')).toHaveCount(0);
