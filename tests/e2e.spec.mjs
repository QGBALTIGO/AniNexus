...
test('mobile drawer prioritizes the account, library and compact destinations',async({page},testInfo)=>{
  ...
  const accountAlignment=await auth.evaluate(element=>{const avatar=element.querySelector('.drawer-account-avatar').getBoundingClientRect(),copy=element.querySelector('.drawer-auth-copy').getBoundingClientRect(),description=element.querySelector('.drawer-auth-description').getBoundingClientRect(),profile=element.querySelector('.drawer-profile-link').getBoundingClientRect(),tools=element.querySelector('.drawer-account-tools').getBoundingClientRect(),box=element.getBoundingClientRect();return{avatarTop:Math.abs(avatar.top-copy.top),toolsTop:Math.abs(tools.top-copy.top),profileLeft:Math.abs(profile.left-box.left),profileRight:Math.abs(box.right-profile.right),profileBelowHandle:profile.top>=description.bottom-1,left:avatar.left-box.left,right:box.right-tools.right}});
  expect(accountAlignment.avatarTop).toBeLessThanOrEqual(2);
  expect(accountAlignment.toolsTop).toBeLessThanOrEqual(2);
  expect(accountAlignment.profileLeft).toBeLessThanOrEqual(3);
  expect(accountAlignment.profileRight).toBeLessThanOrEqual(3);
  expect(accountAlignment.profileBelowHandle).toBe(true);
