...
(() => {
  ...
  function homeImpCard(x){const name=x.username||'membro',m=x.media||mediaOf(null,x.mediaId),status=x.status?STATUS[x.status]:null,score=x.score!=null?Number(x.score).toFixed(1).replace('.0',''):null;return `<article class="nx38-impression-home-card" ${m.id?`data-lib-home-open="${m.id}" data-title="${esc(m.title)}"`:''}><div class="nx38-impression-user"><span class="nx38-impression-avatar">${x.avatarUrl?`<img src="${esc(x.avatarUrl)}" alt="">`:esc(name.charAt(0).toUpperCase())}</span><strong>@${esc(name)}</strong><time>${esc(fmtRel(x.createdAt))}</time></div><p class="nx38-impression-body${x.spoiler?' spoiler':''}">${x.spoiler?'Impressão marcada como spoiler. Abra o anime para ler com contexto.':esc(x.body)}</p><div class="nx38-impression-meta-row">${status?`<span class="status">${esc(status)}</span>`:''}${x.progress?`<span>no episódio ${x.progress}</span>`:''}${score?`<span class="score">★ ${score}</span>`:''}</div><div class="nx38-impression-context">${m.cover?`<img src="${esc(m.cover)}" alt="${esc(m.title)}" loading="lazy" decoding="async">`:'<div class="nx38-impression-cover-fallback">AN</div>'}<div><strong>${esc(m.title)}</strong><small>Ver anime e impressões</small></div><i aria-hidden="true">→</i></div></article>`}
