...
(() => {
  ...
  function mediaOf(raw,id){
    ...
    const internal=m.metricsSource==='aninexus',score=internal&&m.score!=null?Number(m.score):null,mediaId=Number(m.id||id),titleSlug=slug(title),routeSlug=String(m.slug||'').trim()||(titleSlug.endsWith(`-${mediaId}`)?titleSlug:`${titleSlug}-${mediaId}`);
    return {id:mediaId,title,cover,banner,score:Number.isFinite(score)?score:null,metricsSource:internal?'aninexus':'',episodes:m.episodes==null?null:Number(m.episodes),status:String(m.status||''),format:String(m.format||''),seasonYear:m.seasonYear||null,genres:Array.isArray(m.genres)?m.genres:[],slug:routeSlug};
  }
  function normalizeRow(row){const id=Number(row?.media_id||row?.mediaId||row?.media?.id);return{id,status:String(row?.status||''),score:row?.score==null?null:Number(row.score),reaction:row?.reactions?.[0]||row?.reaction||'',reactions:row?.reactions||[],progress:Math.max(0,Number(row?.progress)||0),updatedAt:Date.parse(row?.updated_at||row?.updatedAt||0)||0,media:mediaOf(row?.media,id)}}
  ...
  function normalizeImp(row){const id=Number(row?.media_id||row?.mediaId||row?.media?.id);return{id:String(row?.id||crypto?.randomUUID?.()||Date.now()),mediaId:id,mediaType:String(row?.media_type||row?.mediaType||'ANIME').toUpperCase()==='MANGA'?'MANGA':'ANIME',body:String(row?.body||''),spoiler:!!row?.spoiler,createdAt:row?.created_at||row?.createdAt||new Date().toISOString(),username:String(row?.username||'você'),displayName:String(row?.display_name||row?.displayName||row?.username||'você'),avatarUrl:row?.avatar_url||row?.avatarUrl||'',status:String(row?.status||''),score:row?.score==null?null:Number(row.score),reaction:String(row?.reactions?.[0]||row?.reaction||''),progress:Math.max(0,Number(row?.progress)||0),media:mediaOf(row?.media,id)}}
  ...
  function homeImpCard(x){
    const username=x.username||'membro',displayName=x.displayName||username,m=x.media||mediaOf(null,x.mediaId),type=x.mediaType==='MANGA'?'MANGA':'ANIME',status=x.status?(type==='MANGA'?{CURRENT:'Lendo',PLANNING:'Quero ler',COMPLETED:'Terminei',PAUSED:'Pausei',DROPPED:'Desisti'}[x.status]:STATUS[x.status]):null,score=x.score!=null&&Number.isFinite(Number(x.score))?Number(x.score).toFixed(1).replace('.0',''):null,kind=type==='MANGA'?'Mangá':format(m.format),baseSlug=String(m.slug||slug(m.title)).replace(new RegExp(`-${m.id}$`),'')||slug(m.title),mediaPath=`/${type==='MANGA'?'manga':'anime'}/${baseSlug}-${m.id}`,profilePath=username&&!['membro','você'].includes(username.toLowerCase())?`/u/${username}`:'',details=[kind,m.seasonYear,m.episodes?`${m.episodes} ep.`:null].filter(Boolean).join(' · '),tone=['CURRENT','PLANNING','COMPLETED','PAUSED','DROPPED'].includes(x.status)?x.status.toLowerCase():'neutral',reaction={LIKE:'Gostei',DISLIKE:'Não curti',LOVE:'Amei',WOW:'Me surpreendeu'}[x.reaction]||x.reaction,date=Date.parse(x.createdAt),dateTitle=Number.isFinite(date)?new Intl.DateTimeFormat('pt-BR',{dateStyle:'long',timeStyle:'short'}).format(new Date(date)):'';
    const avatar=`<span class="nx38-impression-avatar">${x.avatarUrl?`<img src="${esc(x.avatarUrl)}" alt="" loading="lazy" decoding="async">`:esc(displayName.charAt(0).toUpperCase())}</span>`;
    const identity=`${avatar}<span class="nx38-impression-identity"><strong>${esc(displayName)}</strong><small>@${esc(username)}</small></span>`;
    const author=profilePath?`<a class="nx38-impression-author" href="${pageUrl(profilePath)}" data-nx38-nav="${esc(profilePath)}" aria-label="Ver perfil de ${esc(displayName)}">${identity}</a>`:`<span class="nx38-impression-author">${identity}</span>`;
    return `<article class="nx38-impression-home-card status-${tone}">${m.banner||m.cover?`<img class="nx38-impression-art" src="${esc(m.banner||m.cover)}" alt="" loading="lazy" decoding="async" aria-hidden="true">`:''}<header class="nx38-impression-user">${author}<time datetime="${esc(x.createdAt)}" title="${esc(dateTitle)}">${esc(fmtRel(x.createdAt))}</time></header><p class="nx38-impression-body${x.spoiler?' spoiler':''}">${x.spoiler?'<span>Impressão com spoiler</span><small>Abra a obra para ler com contexto.</small>':esc(x.body)}</p><div class="nx38-impression-meta-row" aria-label="Contexto da impressão">${status?`<span class="status">${esc(status)}</span>`:''}${x.progress?`<span class="progress">${type==='MANGA'?'capítulo':'episódio'} ${x.progress}${type==='ANIME'&&m.episodes?` de ${m.episodes}`:''}</span>`:''}${reaction?`<span class="reaction">${esc(reaction)}</span>`:''}${score?`<span class="score">★ ${score}</span>`:''}${!x.spoiler?'<span class="spoiler-free">sem spoilers</span>':''}</div><a class="nx38-impression-context" href="${pageUrl(mediaPath)}" data-nx38-nav="${esc(mediaPath)}">${m.cover?`<img src="${esc(m.cover)}" alt="Capa de ${esc(m.title)}" loading="lazy" decoding="async">`:`<span class="nx38-impression-cover-fallback">AN</span>`}<span class="nx38-impression-media-copy"><small>${esc(details||kind)}</small><strong>${esc(m.title)}</strong><em>Ver obra e outras impressões</em></span><i aria-hidden="true">→</i></a></article>`
  }
  async function mountHomeImpressions(force=false){
    ...
    const news=document.querySelector('#nx35News')?.closest('.nx35-section'),host=document.createElement('section');host.id='nx38HomeImpressions';host.className='nx38-impressions-home tonal';host.innerHTML=`<div class="nx35-shell"><div class="nx38-impressions-head"><div><span class="nx38-impressions-kicker">COMUNIDADE</span><h2>Últimas <em>Impressões</em></h2><p>Opiniões de quem está assistindo, com episódio, progresso e nota no mesmo contexto.</p></div><div class="nx38-impressions-actions"><button type="button" data-imp-prev aria-label="Impressão anterior">‹</button><button type="button" data-imp-next aria-label="Próxima impressão">›</button><a href="${pageUrl('/comunidade')}">Abrir comunidade</a></div></div><div class="nx38-impressions-empty">Carregando impressões…</div></div>`;if(news)news.insertAdjacentElement('afterend',host);else home.append(host);
    const items=await loadPublicImpressions(),body=host.querySelector('.nx38-impressions-empty');if(!document.body.contains(host))return;host.dataset.count=String(items.length);if(items.length){body.outerHTML=`<div class="nx38-impressions-rail" data-count="${items.length}" tabindex="0" aria-label="Últimas impressões da comunidade">${items.map(homeImpCard).join('')}</div>`;const rail=host.querySelector('.nx38-impressions-rail'),prev=host.querySelector('[data-imp-prev]'),next=host.querySelector('[data-imp-next]'),update=()=>{const max=Math.max(0,rail.scrollWidth-rail.clientWidth);prev.disabled=rail.scrollLeft<8;next.disabled=rail.scrollLeft>max-8};prev.onclick=()=>rail.scrollBy({left:-Math.max(280,rail.clientWidth*.8),behavior:'smooth'});next.onclick=()=>rail.scrollBy({left:Math.max(280,rail.clientWidth*.8),behavior:'smooth'});rail.addEventListener('scroll',update,{passive:true});requestAnimationFrame(update)}else{body.innerHTML='Ainda não há impressões públicas. Quando a comunidade começar a publicar, elas aparecem aqui automaticamente.';host.querySelectorAll('[data-imp-prev],[data-imp-next]').forEach(button=>button.disabled=true)}
    host.querySelectorAll('[data-nx38-nav]').forEach(link=>link.addEventListener('click',event=>{if(event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;event.preventDefault();go(link.dataset.nx38Nav)}));
  }
