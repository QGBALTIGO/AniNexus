...
(() => {
  ...
  const routeUrl = path => IS_PAGES ? `${BASE}/?build=44.59.0&p=${encodeURIComponent(path)}` : path;
  ...
  function sourceConnectionCard(connection={},feedback='',pending=false){
    const linked=connection?.linked===true,profile=connection?.profile||{},stats=profile?.stats||{},favorite=profile?.favorite||null;
    if(pending)return `<section class="nx56-panel nx56-source-card" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-section-head"><div><small>CONFIRMAÇÃO</small><h2 id="nx56SourceTitle">Conectar Source</h2><p>O Source ainda não foi vinculado. Confirme somente se esta é a conta AniNexus que você quer usar.</p></div><span class="nx56-source-state">Pendente</span></header>${feedback?`<p class="nx56-source-feedback" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-empty">${accountIcon('link')}<div><strong>Conectar à conta AniNexus atual?</strong><span>Ao confirmar, o perfil Source será associado à conta exibida no topo desta página. Nenhuma senha ou sessão do Telegram é compartilhada.</span></div></div><div class="nx56-source-actions"><button type="button" data-source-confirm>${accountIcon('check')}<span>Conectar a esta conta</span></button><button type="button" data-source-cancel><span>Cancelar</span></button></div></section>`;
    if(!linked)return `<section class="nx56-panel nx56-source-card" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-section-head"><div><small>INTEGRAÇÃO</small><h2 id="nx56SourceTitle">Source Baltigo</h2><p>Conecte sua conta do bot para exibir coleção, nível e personagem favorito no AniNexus.</p></div><span class="nx56-source-state is-offline">Não conectado</span></header>${feedback?`<p class="nx56-source-feedback" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-empty">${accountIcon('link')}<div><strong>Conexão iniciada pelo Source</strong><span>Abra o Source no Telegram, vá em Configurações → AniNexus e toque em “Conectar AniNexus”. O link expira rapidamente e funciona uma única vez.</span></div></div></section>`;
    const visibility=connection.publicVisible!==false,sourcePublic=profile.public!==false;
    return `<section class="nx56-panel nx56-source-card is-linked" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-section-head"><div><small>INTEGRAÇÃO</small><h2 id="nx56SourceTitle">Source Baltigo</h2><p>Seu progresso do bot está ligado a esta conta AniNexus.</p></div><span class="nx56-source-state is-online">${connection.stale?'Conectado · cache':'Conectado'}</span></header>${feedback?`<p class="nx56-source-feedback is-success" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-profile"><div class="nx56-source-character">${favorite?.image?`<img src="${esc(favorite.image)}" alt="${esc(favorite.name||'Personagem favorito')}" loading="lazy">`:`<span>${accountIcon('link')}</span>`}</div><div class="nx56-source-copy"><strong>${esc(profile.displayName||'Conta Source')}</strong><small>${profile.username?`@${esc(profile.username)} · `:''}Nível ${Number(stats.level||1)}</small>${favorite?`<p><b>${esc(favorite.name)}</b>${favorite.work?` · ${esc(favorite.work)}`:''}</p>`:''}</div><div class="nx56-source-stats"><span><b>${Number(stats.uniqueCharacters||0).toLocaleString('pt-BR')}</b>personagens</span><span><b>${Number(stats.collectionPercent||0).toLocaleString('pt-BR',{maximumFractionDigits:1})}%</b>coleção</span><span><b>${Number(stats.xp||0).toLocaleString('pt-BR')}</b>XP</span></div></div><div class="nx56-source-actions"><button type="button" data-source-visibility aria-pressed="${visibility?'true':'false'}">${accountIcon(visibility?'eye':'shield')}<span>${visibility?'Visível no perfil':'Oculto no perfil'}</span></button><button type="button" class="danger" data-source-disconnect>${accountIcon('logout')}<span>Desconectar</span></button></div>${!sourcePublic?'<p class="nx56-source-note">Seu perfil está privado no Source; por isso estes dados não aparecem publicamente mesmo que a opção acima esteja ativa.</p>':''}</section>`;
  }
  function bindSourceConnection(){
    ...
    if(confirmLink)confirmLink.onclick=async()=>{
      ...
      try{await api('/api/me/source/link',{method:'POST',body:JSON.stringify({token}),timeout:10_000});sessionStorage.removeItem(SOURCE_LINK_PENDING);sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'Conta Source conectada com sucesso.');await renderAccount()}
