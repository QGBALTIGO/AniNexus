...
(() => {
  ...
  const accountIcon = name => ({
    ...
    link:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 14a4 4 0 0 0 5.7 0l2.3-2.3A4 4 0 0 0 12.3 6l-1.2 1.2"/><path d="M14 10a4 4 0 0 0-5.7 0L6 12.3A4 4 0 0 0 11.7 18l1.2-1.2"/></svg>',
  ...
  const SOURCE_LINK_PENDING='aninexus:pending-source-link-v1';
  const SOURCE_FOCUS_KEY='aninexus:focus-source-v1';
  const SOURCE_FEEDBACK_KEY='aninexus:source-feedback-v1';
  function sourceTokenFromLocation(){try{const url=new URL(location.href),fragment=new URLSearchParams(url.hash.replace(/^#/,'')),fragmentToken=fragment.get('source_token');if(fragmentToken)return fragmentToken;const restored=url.searchParams.get('p');if(restored&&restored.includes('?')){const query=new URLSearchParams(restored.slice(restored.indexOf('?')+1)),token=query.get('token');if(token)return token}return url.searchParams.get('token')||''}catch{return''}}
  async function resolveSourceConnection(){
    const pending=Boolean(sessionStorage.getItem(SOURCE_LINK_PENDING));
    const feedback=sessionStorage.getItem(SOURCE_FEEDBACK_KEY)||'';
    if(feedback)sessionStorage.removeItem(SOURCE_FEEDBACK_KEY);
    try{return{connection:await api('/api/me/source',{timeout:7_000}),feedback,pending}}
    catch(error){console.warn('[AniNexus Source] estado do vínculo indisponível.',error);return{connection:{linked:false,unavailable:true},feedback:feedback||'O Source está temporariamente indisponível. Suas outras funções do AniNexus continuam normais.',pending}}
  }
  function sourceConnectionCard(connection={},feedback='',pending=false){
    const linked=connection?.linked===true,profile=connection?.profile||{},stats=profile?.stats||{},favorite=profile?.favorite||null,reward=profile?.integration?.reward||{};
    const rewardCopy=reward?.claimed
      ? `Bônus recebido · +${Number(reward.coinsGranted||reward.coins||50)} Coins${Number(reward.dadosGranted??reward.dados??1)>0?` · +${Number(reward.dadosGranted??reward.dados??1)} Dado`:''}`
      : 'Bônus de conexão · +50 Coins · +1 Dado';
    if(pending)return `<section class="nx56-panel nx56-source-card is-pending" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-source-head"><div><small>CONFIRMAÇÃO</small><h2 id="nx56SourceTitle">Source AniNexus</h2><p>Confirme a conta AniNexus antes de concluir o vínculo.</p></div><span class="nx56-source-state">Pendente</span></header>${feedback?`<p class="nx56-source-feedback" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-empty">${accountIcon('link')}<div><strong>Conectar à conta atual?</strong><span>O Source será associado à conta AniNexus exibida no topo. Senha e sessão do Telegram não são compartilhadas.</span></div></div><div class="nx56-source-reward"><b>Primeira conexão</b><span>+50 Coins</span><span>+1 Dado</span><small>recompensa única por conta Source</small></div><div class="nx56-source-actions"><button type="button" class="primary" data-source-confirm>${accountIcon('check')}<span>Conectar a esta conta</span></button><button type="button" data-source-cancel><span>Cancelar</span></button></div></section>`;
    if(!linked)return `<section class="nx56-panel nx56-source-card" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-source-head"><div><small>INTEGRAÇÃO</small><h2 id="nx56SourceTitle">Source AniNexus</h2><p>Leve seu perfil, nível, favorito e coleção do Source para o AniNexus.</p></div><span class="nx56-source-state is-offline">Não conectado</span></header>${feedback?`<p class="nx56-source-feedback" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-empty">${accountIcon('link')}<div><strong>Conecte pelo bot Source</strong><span>No Telegram, abra /aninexus ou Configurações → Source AniNexus. O link é temporário e funciona uma única vez.</span></div></div><div class="nx56-source-reward"><b>Missão de integração</b><span>+50 Coins</span><span>+1 Dado</span><small>inclui badge Source AniNexus</small></div></section>`;
    const visibility=connection.publicVisible!==false,sourcePublic=profile.public!==false;
    return `<section class="nx56-panel nx56-source-card is-linked" id="source" aria-labelledby="nx56SourceTitle"><header class="nx56-source-head"><div><small>INTEGRAÇÃO</small><h2 id="nx56SourceTitle">Source AniNexus</h2><p>Seu progresso do Source está sincronizado com esta conta.</p></div><span class="nx56-source-state is-online">${connection.stale?'Conectado · cache':'Conectado'}</span></header>${feedback?`<p class="nx56-source-feedback is-success" role="status">${esc(feedback)}</p>`:''}<div class="nx56-source-profile"><div class="nx56-source-character">${favorite?.image?`<img src="${esc(favorite.image)}" alt="${esc(favorite.name||'Personagem favorito')}" loading="lazy">`:`<span>${accountIcon('link')}</span>`}</div><div class="nx56-source-copy"><strong>${esc(profile.displayName||'Conta Source')}</strong><small>${profile.username?`@${esc(profile.username)} · `:''}Nível ${Number(stats.level||1)}</small>${favorite?`<p><b>${esc(favorite.name)}</b>${favorite.work?` · ${esc(favorite.work)}`:''}</p>`:''}</div><div class="nx56-source-badge">${accountIcon('link')}<span>Source AniNexus</span></div></div><div class="nx56-source-metrics"><article><small>PERSONAGENS</small><b>${Number(stats.uniqueCharacters||0).toLocaleString('pt-BR')}</b></article><article><small>COLEÇÃO</small><b>${Number(stats.collectionPercent||0).toLocaleString('pt-BR',{maximumFractionDigits:1})}%</b></article><article><small>XP</small><b>${Number(stats.xp||0).toLocaleString('pt-BR')}</b></article></div><div class="nx56-source-reward is-claimed"><b>${esc(rewardCopy)}</b><small>${reward?.claimed?'A recompensa desta conta não pode ser recebida novamente.':'A recompensa fica disponível no Source.'}</small></div><div class="nx56-source-actions"><button type="button" data-source-visibility aria-pressed="${visibility?'true':'false'}">${accountIcon(visibility?'eye':'shield')}<span>${visibility?'Visível no perfil':'Oculto no perfil'}</span></button><button type="button" data-source-sync>${accountIcon('system')}<span>Sincronizar agora</span></button><button type="button" class="danger" data-source-disconnect>${accountIcon('logout')}<span>Desconectar</span></button></div>${!sourcePublic?'<p class="nx56-source-note">Seu perfil está privado no Source. Estes dados ficam visíveis somente para você até alterar essa preferência no bot.</p>':''}</section>`;
  }
  function bindSourceConnection(){
    const confirmLink=document.querySelector('[data-source-confirm]');
    if(confirmLink)confirmLink.onclick=async()=>{
      const token=sessionStorage.getItem(SOURCE_LINK_PENDING);
      if(!token){sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'O link do Source não está mais disponível. Gere outro pelo bot.');await renderAccount();return}
      confirmLink.disabled=true;const cancel=document.querySelector('[data-source-cancel]');if(cancel)cancel.disabled=true;
      try{const result=await api('/api/me/source/link',{method:'POST',body:JSON.stringify({token}),timeout:10_000});sessionStorage.removeItem(SOURCE_LINK_PENDING);const reward=result?.profile?.integration?.reward||result?.reward||{};const rewardText=reward?.claimed?` Recompensa: +${Number(reward.coinsGranted||reward.coins||50)} Coins${Number(reward.dadosGranted??reward.dados??1)>0?` e +${Number(reward.dadosGranted??reward.dados??1)} Dado`:''}.`:'';sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'Conta Source conectada com sucesso.'+rewardText);await renderAccount()}
      catch(error){const terminal=[400,409,410,422].includes(Number(error?.status||0));if(terminal)sessionStorage.removeItem(SOURCE_LINK_PENDING);sessionStorage.setItem(SOURCE_FEEDBACK_KEY,Number(error?.status)===410?'O link do Source expirou ou já foi usado. Gere outro pelo bot.':'Não foi possível concluir o vínculo agora. Nenhuma conta foi alterada.');await renderAccount()}
    };
    const cancelLink=document.querySelector('[data-source-cancel]');if(cancelLink)cancelLink.onclick=async()=>{sessionStorage.removeItem(SOURCE_LINK_PENDING);sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'Conexão cancelada. Nenhuma conta foi vinculada.');await renderAccount()};
    const visibility=document.querySelector('[data-source-visibility]');if(visibility)visibility.onclick=async()=>{visibility.disabled=true;try{await api('/api/me/source',{method:'PATCH',body:JSON.stringify({publicVisible:visibility.getAttribute('aria-pressed')!=='true'})});await renderAccount()}catch(error){console.warn('[AniNexus Source] preferência não atualizada.',error);visibility.disabled=false}};
    const sync=document.querySelector('[data-source-sync]');if(sync)sync.onclick=async()=>{sync.disabled=true;try{await api('/api/me/source',{timeout:8_000});sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'Dados do Source sincronizados agora.');await renderAccount()}catch(error){console.warn('[AniNexus Source] sincronização falhou.',error);sessionStorage.setItem(SOURCE_FEEDBACK_KEY,'Não foi possível sincronizar agora. Seus dados anteriores foram preservados.');await renderAccount()}};
    const disconnect=document.querySelector('[data-source-disconnect]');if(disconnect)disconnect.onclick=async()=>{if(!confirm('Desconectar sua conta Source do AniNexus? Você poderá conectar novamente depois.'))return;disconnect.disabled=true;try{await api('/api/me/source',{method:'DELETE'});await renderAccount()}catch(error){console.warn('[AniNexus Source] não foi possível desconectar.',error);disconnect.disabled=false}};
    const shouldFocus=location.hash==='#source'||sessionStorage.getItem(SOURCE_FOCUS_KEY)==='1';
    if(shouldFocus){sessionStorage.removeItem(SOURCE_FOCUS_KEY);requestAnimationFrame(()=>document.querySelector('#source')?.scrollIntoView({block:'start'}))}
  }
  ...
  async function renderAccount() {
    ...
    try {
      ...
      const [me, list, follows, notes, importStatus, connections, sourceState] = await Promise.all([api('/api/me'), api('/api/me/list'), api('/api/me/follows'), api('/api/me/notifications?limit=100'), api('/api/me/import-status'), api('/api/me/profile-connections'), resolveSourceConnection()]);
  ...
  async function renderSourceConnect(){
    activate();document.title='Conectar Source | AniNexus';
    const token=String(sourceTokenFromLocation()||'').trim();
    if(token&&token.length>=20&&token.length<=256)sessionStorage.setItem(SOURCE_LINK_PENDING,token);
    const pending=sessionStorage.getItem(SOURCE_LINK_PENDING);
    if(!pending){app.innerHTML='<main class="nx38-account-page"><div class="nx38-account-shell"><div class="nx38-pages-note">Este link do Source é inválido ou não está mais disponível. Gere um novo pelo bot.</div></div></main>';return}
    if(!ENABLED){app.innerHTML=`<main class="nx38-account-page"><div class="nx38-account-shell">${unavailableCard()}</div></main>`;return}
    app.innerHTML='<main class="nx38-account-page"><div class="nx38-account-shell"><div class="nx38-auth-card" style="margin:auto"><i class="nx38-spin"></i><span class="sr-only">Preparando conexão com o Source</span></div></div></main>';
    try{const clerk=await loadClerk();if(!clerk?.user){go('/login',true);return}sessionStorage.setItem(SOURCE_FOCUS_KEY,'1');go('/minha-conta',true)}catch{app.innerHTML='<main class="nx38-account-page"><div class="nx38-account-shell"><div class="nx38-pages-note">Não foi possível preparar sua conta agora. O link do Source foi preservado neste dispositivo para uma nova tentativa.</div></div></main>'}
  }
  function mountRoute() {
