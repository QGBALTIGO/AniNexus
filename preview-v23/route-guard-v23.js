...
(() => {
  try {
    ...
    const routePathFrom=value=>{try{const current=new URL(value,location.href),restoredPath=current.searchParams.get('p');let next=restoredPath?restoredPath.split('?')[0]:current.pathname;if(IS_PAGES&&!restoredPath)next=next.replace(/^\/AniNexus/,'')||'/';return String(next||'/').replace(/\/+$/,'')||'/'}catch{return''}};
    ...
    const installNavigation=()=>{
      ...
      const go=(value,options={})=>{let target;try{target=new URL(value,location.href)}catch{return false}if(target.origin!==location.origin)return false;const {replace=false,scroll='top',popstate=true,event='aninexus:navigate',state={}}=options;document.body?.classList.remove('modal-open','nx22-tabs-open','nx-popover-open','nx-v11-fixed-popover');const drawer=document.querySelector('#drawer');if(drawer){drawer.hidden=true;drawer.setAttribute('aria-hidden','true')}history[replace?'replaceState':'pushState'](state,'',`${target.pathname}${target.search}${target.hash}`);if(scroll==='top'&&!target.hash)restore({x:0,y:0});if(popstate)dispatchEvent(new PopStateEvent('popstate',{state:history.state}));if(event)dispatchEvent(new CustomEvent(event,{detail:{route:`${target.pathname}${target.search}${target.hash}`}}));requestAnimationFrame(()=>document.querySelector('#app')?.focus?.({preventScroll:true}));return true};
