...
(() => {
  ...
  function helpers(){if(document.querySelector('script[data-nx23-news-i18n]'))return;const s=document.createElement('script');s.src=`${BASE}/preview-v23/news-i18n-v23.js?v=${BUILD}`;s.defer=true;s.dataset.nx23NewsI18n='1';document.head.append(s)}
  rewrite();helpers();addEventListener('DOMContentLoaded',()=>rewrite(),{once:true});new MutationObserver(rs=>{for(const r of rs)for(const n of r.addedNodes)if(n.nodeType===1)rewrite(n)}).observe(document.documentElement,{subtree:true,childList:true});
