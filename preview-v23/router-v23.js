...
(() => {
  ...
  const BUILD='44.37.0'; // Cache lineage: BUILD='44.35.3'.
  ...
  function markDetailOwner(path){const route=String(path||'').split(/[?#]/)[0];window.__NX_ROUTE_OWNER__='detail';window.__NX_DEDICATED_BOOT_PATH__=route;window.__NX_USE_V22_DETAIL__=true;window.__NX_DETAIL_ROLLBACK_PATH__=route}
  function ensureDetailRuntime(){
    ...
    detailRuntimePromise=new Promise((resolve,reject)=>{
      ...
      const finish=error=>{if(settled)return;settled=true;clearTimeout(timer);removeEventListener('aninexus:detail-runtime-ready',ready);runtime?.removeEventListener('load',loaded);runtime?.removeEventListener('error',failed);if(error)reject(error);else resolve()};
      const ready=()=>finish();
      ...
      if(!runtime){runtime=document.createElement('script');runtime.src=`${BASE}/preview-v22/detail-v22.js?v=${BUILD}`;runtime.async=false;runtime.dataset.nx22DetailRuntime='1'}
  ...
  function navigate(path){
    ...
    if(radioNavigate&&window.AniNexusRadio.navigate(path))return;
    assign(path);
  }
