...
(() => {
  ...
  const BUILD='40.8.0';
  const DEDICATED=['/','/animes/catalogo','/animes/programacao','/animes/temporadas','/noticias','/comunidade','/login','/criar-conta','/minha-conta','/meus-animes','/admin'];
  ...
  function cleanPathFromUrl(href){try{const u=new URL(href,location.href);const restored=u.searchParams.get('p');if(restored)return restored.split('?')[0].replace(/\/+$/,'')||'/';let p=u.pathname;if(IS_PAGES)p=p.replace(/^\/AniNexus(?:\/AniNexus)?/,'')||'/';return p.replace(/\/+$/,'')||'/'}catch{return''}}
  function isDedicated(p){return DEDICATED.includes(p)||/^\/u\/[\p{L}\p{N}_.-]{3,30}$/u.test(p)||/^\/animes\/temporadas\//.test(p)||/^\/anime\/.+-\d+$/.test(p)||/^\/manga\/.+-\d+$/.test(p)||/^\/noticias\/[a-z0-9-]+$/.test(p)}
  ...
  function navigate(path){document.body?.classList.remove('modal-open');const drawer=document.querySelector('#drawer');if(drawer){drawer.hidden=true;drawer.setAttribute('aria-hidden','true')}location.assign(IS_PAGES?pagesUrl(path):path)}
