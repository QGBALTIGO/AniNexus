...
(() => {
  ...
  function navigate(path){
    ...
    location.assign(IS_PAGES?pagesUrl(path):path);
  }
  ...
  addEventListener('keydown',e=>{
    ...
    const card=e.target.closest?.(CARD_SELECTOR);if(!card||e.target.closest(ACTION_SELECTOR))return;
    const info=cardInfo(card);if(!info)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();navigate(info.path);
  },true);
})();