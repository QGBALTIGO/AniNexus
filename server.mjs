...
app.post('/api/news/:slug/comments',rateForUser(5,'1 minute','news-comments-write'),async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const slug=String(req.params.slug||'').slice(0,180),parsed=z.object({body:z.string().trim().min(2).max(1800),spoiler:z.boolean().optional(),parentId:z.string().uuid().nullable().optional()}).safeParse(req.body);if(!parsed.success)return reply.code(400).send({error:'INVALID_INPUT'});const article=await getNativeArticle(slug);if(!article)return reply.code(404).send({error:'NOT_FOUND'});const normalized=parsed.data.body.normalize('NFKC');if(/https?:\/\/\S+/gi.test(normalized)&&((normalized.match(/https?:\/\/\S+/gi)||[]).length>2))return reply.code(422).send({error:'TOO_MANY_LINKS'});const parentId=parsed.data.parentId||null,{rows}=await q(`WITH new_id AS (SELECT gen_random_uuid() id),parent AS (SELECT id,root_id,depth FROM news_comments WHERE id=$3::uuid AND article_slug=$1 AND hidden=false) INSERT INTO news_comments(id,article_slug,user_id,parent_id,root_id,depth,body,spoiler) SELECT new_id.id,$1,$2,$3::uuid,CASE WHEN $3::uuid IS NULL THEN new_id.id ELSE COALESCE(parent.root_id,parent.id) END,CASE WHEN $3::uuid IS NULL THEN 0 ELSE parent.depth+1 END,$4,$5 FROM new_id LEFT JOIN parent ON true WHERE $3::uuid IS NULL OR parent.id IS NOT NULL RETURNING id,created_at`,[slug,u.id,parentId,normalized,parsed.data.spoiler||false]);if(!rows[0])return reply.code(422).send({error:'INVALID_PARENT'});await recordContributionAchievement(u.id,'NEWS_COMMENT',rows[0],normalized);return reply.code(201).send({ok:true,...rows[0]});});
...
registerSocialRoutes(app,{q,currentUser,requireUser,rateForUser,publicRate,safeInt,withActorAvatars,hydrateCommunityMedia,mediaProjection,recordContributionAchievement});
