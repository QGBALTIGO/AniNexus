...
app.addHook('onSend',async(req,reply,payload)=>{
  ...
  else if(req.method==='GET'&&/^\/api\/(catalog|reading|schedule|anime\/\d+|manga\/\d+|studios|dublados|lists|list\/|news(?:\/|$))/.test(url))reply.header('Cache-Control','public, max-age=20, stale-while-revalidate=180, stale-if-error=600');
