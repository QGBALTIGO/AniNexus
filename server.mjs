...
app.put('/api/me/character-favorites/:characterId',writeRate,async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const characterId=safeInt(req.params.characterId);if(!characterId)return reply.code(400).send({error:'INVALID_ID'});const result=await setCharacterFavorite(u.id,characterId,true);if(!result)return reply.code(404).send({error:'CHARACTER_NOT_FOUND'});return{ok:true,...result};});
...
app.post('/api/community/threads',rateForUser(4,'10 minutes','threads-write'),async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const p=z.object({mediaId:z.number().int().positive().nullable().optional(),title:z.string().trim().min(3).max(180),body:z.string().trim().min(3).max(5000),spoiler:z.boolean().optional()}).safeParse(req.body);if(!p.success)return reply.code(400).send({error:'INVALID_INPUT'});const {rows}=await q('INSERT INTO community_threads(user_id,media_id,title,body,spoiler) VALUES($1,$2,$3,$4,$5) RETURNING id,created_at',[u.id,p.data.mediaId||null,p.data.title,p.data.body,p.data.spoiler||false]);await recordContributionAchievement(u.id,'THREAD',rows[0],p.data.body);return reply.code(201).send({ok:true,...rows[0]})});
app.post('/api/community/threads/:id/posts',rateForUser(10,'5 minutes','posts-write'),async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const id=z.string().uuid().safeParse(req.params.id),p=z.object({parentId:z.string().uuid().nullable().optional(),body:z.string().trim().min(1).max(3000),spoiler:z.boolean().optional()}).safeParse(req.body);if(!id.success||!p.success)return reply.code(400).send({error:'INVALID_INPUT'});const {rows:threadRows}=await q('SELECT locked FROM community_threads WHERE id=$1 AND hidden=false',[id.data]);if(!threadRows[0])return reply.code(404).send({error:'NOT_FOUND'});if(threadRows[0].locked)return reply.code(423).send({error:'THREAD_LOCKED'});const {rows}=await q('INSERT INTO community_posts(thread_id,user_id,parent_id,body,spoiler) VALUES($1,$2,$3,$4,$5) RETURNING id,created_at',[id.data,u.id,p.data.parentId||null,p.data.body,p.data.spoiler||false]);await recordContributionAchievement(u.id,'POST',rows[0],p.data.body);return reply.code(201).send({ok:true,...rows[0]})});
