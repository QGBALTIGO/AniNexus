...
(() => {
  ...
  function looksPortuguese(s){const text=String(s||''),pt=languageHits(text,['uma','que','para','com','seu','sua','quando','após','mundo','história','vida','jovem','escola','poder','dos','das','não','como','mas','por','em']),en=languageHits(text,['the','and','with','his','her','when','after','world','story','life','young','school','power','from','into','but']);return(/[áàâãéêíóôõúç]/i.test(text)&&pt>=1)||(pt>=3&&pt>en*1.35)}
  ...
  function paint(m){
    ...
    function wirePanel(k){
      ...
      if(k==='geral')synopsisPT(m).then(text=>{if(mediaId()===m.id){const el=root.querySelector('#nx22Synopsis'),note=root.querySelector('#nx22SynopsisNote');if(el)el.textContent=text;if(note)note.textContent='Em português'}});
