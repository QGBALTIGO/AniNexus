...
(() => {
  ...
  function portrait(n){const name=n?.name?.full||'';return n?.image?.large?`<img loading="lazy" src="${esc(n.image.large)}" alt="${esc(name)}">`:`<i class="nx22-person-fallback" aria-hidden="true">${esc(initials(name))}</i>`}
  ...
  function paint(m){
    ...
    const cast=()=>`<section class="nx22-full nx22-cast-panel"><div class="nx22-detail-rail-section"><div class="nx22-section-head"><div><small>ELENCO</small><h2>Personagens</h2></div></div>${chars.length?`<div class="nx44-rail-frame"><div class="nx22-people nx22-people-full" data-nx22-rail>${chars.map(edge=>personCard(edge,characterState)).join('')}</div></div>`:detailEmpty('Personagens ainda não disponíveis','O catálogo ainda não publicou o elenco desta obra. Assim que os dados forem liberados, eles aparecerão aqui.')}</div><div class="nx22-detail-rail-section nx22-staff-section"><div class="nx22-section-head nx22-staff-head"><div><small>PRODUÇÃO</small><h2>Equipe</h2></div></div>${staff.length?`<div class="nx44-rail-frame"><div class="nx22-people nx22-people-full" data-nx22-rail>${staff.map(staffCard).join('')}</div></div>`:detailEmpty('Equipe ainda não disponível','Criação, direção e produção ainda não foram informadas para esta obra.')}</div></section>`;
    ...
    const franchise=()=>`<section class="nx22-full nx22-detail-rail-section"><div class="nx22-section-head"><div><small>UNIVERSO</small><h2>Franquia e relações</h2></div></div>${rels.length?`<div class="nx44-rail-frame"><div class="nx22-related-grid" data-nx22-rail>${rels.map(relationCard).join('')}</div></div>`:detailEmpty('Franquia ainda não disponível','Ainda não há adaptações, continuações ou obras relacionadas registradas para este título.','relations')}</section>`;
    ...
    function bindCharacterFavorites(){root.querySelectorAll('[data-nx22-character-favorite]').forEach(button=>button.addEventListener('click',async()=>{const id=Number(button.dataset.nx22CharacterFavorite),edge=chars.find(item=>Number(item?.node?.id)===id);if(!edge||characterState.pending.has(id))return;if(!await requireAccount())return;const previous=characterState.favorites.has(id),favorite=!previous;favorite?characterState.favorites.add(id):characterState.favorites.delete(id);characterState.pending.add(id);syncCharacterButtons();try{const node=edge.node,result=await privateJson(`/api/me/character-favorites/${id}`,{method:favorite?'PUT':'DELETE',headers:favorite?{'content-type':'application/json'}:{},body:favorite?JSON.stringify({name:node.name?.full||'',nativeName:node.name?.native||'',image:node.image?.large||'',work:title(m),mediaId:m.id,mediaType:media.type}):undefined});const saved=result?.favorite!==false;characterState.favorites[saved?'add':'delete'](id)}catch{previous?characterState.favorites.add(id):characterState.favorites.delete(id);toast('Não foi possível atualizar o personagem agora')}finally{characterState.pending.delete(id);syncCharacterButtons()}}));hydrateCharacterFavorites()}
