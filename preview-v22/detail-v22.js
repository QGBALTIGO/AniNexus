...
(() => {
  ...
  function validTranslation(source,result){const text=String(result||'').replace(/\s+/g,' ').trim();return text.length>=Math.min(80,source.length*.48)&&looksPortuguese(text)&&text.toLowerCase()!==source.toLowerCase()?text:''}
