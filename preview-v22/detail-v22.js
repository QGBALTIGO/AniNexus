...
(() => {
  ...
  function relationCard(edge){const m=edge?.node;if(!m)return'';const relatedType=String(m.mediaType||m.type||'ANIME').toUpperCase()==='MANGA'?'MANGA':'ANIME';return `<article class="nx22-related" tabindex="0" data-nx22-open="${m.id}" data-nx22-media-type="${relatedType}" data-nx-media="${m.id}"><div class="nx22-related-cover">${cover(m)?`<img loading="lazy" src="${esc(cover(m))}" alt="${esc(title(m))}">`:''}</div><div><small>${esc(REL[edge.relationType]||String(edge.relationType||'Relacionado').replaceAll('_',' '))}</small><strong>${esc(title(m))}</strong><span>${esc(mediaFormatPT(m))}${m.seasonYear?` · ${m.seasonYear}`:''}</span></div></article>`}
  function recommendationCard(node){const m=node?.mediaRecommendation;if(!m)return'';const relatedType=String(m.mediaType||m.type||'ANIME').toUpperCase()==='MANGA'?'MANGA':'ANIME';return `<article class="nx22-rec" tabindex="0" data-nx22-open="${m.id}" data-nx22-media-type="${relatedType}" data-nx-media="${m.id}"><div>${cover(m)?`<img loading="lazy" src="${esc(cover(m))}" alt="${esc(title(m))}">`:''}${m.metricsSource==='aninexus'&&m.averageScore?`<span>${SVG.star}${(m.averageScore/10).toFixed(1)}</span>`:''}</div><strong>${esc(title(m))}</strong><small>${esc((m.genres||[]).slice(0,2).map(g=>GENRE[g]||g).join(' · ')||(relatedType==='MANGA'?'Mangá':'Anime'))}</small></article>`}
  ...
  async function hydrateMediaActivity(type,id){
    ...
    const reading=type==='MANGA',labels={PLANNING:reading?'Quero ler':'Quero ver',CURRENT:reading?'Lendo':'Assistindo',COMPLETED:'Terminei',PAUSED:'Pausei',DROPPED:'Desisti'},icons={PLANNING:'＋',CURRENT:'▶',COMPLETED:'✓',PAUSED:'Ⅱ',DROPPED:'×'};
    try{
      const response=await fetch(`/api/${reading?'manga':'anime'}/${Number(id)}/activity`,{headers:{accept:'application/json'}}),data=response.ok?await response.json():null;
      if(!data)throw new Error('activity unavailable');if(mediaId()!==Number(id)||!host.isConnected)return;
      const statuses=data.statuses||{},reactions=Array.isArray(data.reactions)?data.reactions:[];
      host.dataset.state='ready';host.innerHTML=`<div class="nx22-activity-rows">${Object.entries(labels).map(([key,label])=>`<div class="nx22-activity-row"><i data-status="${key}" aria-hidden="true">${icons[key]}</i><strong>${esc(label)}</strong><span></span><b>${Number(statuses[key])||0}</b></div>`).join('')}</div>${reactions.length?`<div class="nx22-activity-reactions">${reactions.map(item=>`<span data-reaction="${esc(item.key)}"><i aria-hidden="true">${item.key==='LOVE'?'♥':item.key==='LIKE'?'●':item.key==='WOW'?'✦':'—'}</i>${esc(item.label)} <b>${Number(item.percentage)||0}%</b></span>`).join('')}</div>`:`<p class="nx22-activity-empty">As reações da comunidade aparecerão aqui.</p>`}<small class="nx22-activity-total">${Number(data.total)||0} ${Number(data.total)===1?'membro acompanha':'membros acompanham'} esta obra</small>`;
    }catch{if(host.isConnected){host.dataset.state='error';host.innerHTML='<p class="nx22-activity-empty">A atividade não pôde ser carregada agora.</p>'}}
  }
  ...
  function paint(m){
    ...
    root.innerHTML=`<article class="nx22-detail" data-nx22-id="${m.id}" data-nx-media="${m.id}" data-nx22-type="${media.type}">
      ...
      <nav class="nx22-tabs" aria-label="Seções da obra">
        <div class="nx22-shell nx22-tabs-bar"><button type="button" class="nx22-tabs-toggle" data-nx22-tabs-toggle aria-expanded="false"><span>SEÇÕES DA OBRA</span><strong data-nx22-current-tab>Visão geral</strong><i aria-hidden="true">⌄</i></button></div>
        <button type="button" class="nx22-tabs-backdrop" data-nx22-tabs-close aria-label="Fechar menu de seções"></button>
        <section class="nx22-tabs-sheet" aria-label="Menu de seções"><header><strong>Seções da obra</strong><button type="button" data-nx22-tabs-close aria-label="Fechar menu">×</button></header><div class="nx22-shell nx22-tab-list" role="tablist"><button class="active" role="tab" aria-selected="true" data-nx22-tab="geral">Visão geral</button>${reading?'':`<button role="tab" aria-selected="false" data-nx22-tab="aberturas">Aberturas & encerramentos</button>`}<button role="tab" aria-selected="false" data-nx22-tab="elenco">Personagens & equipe</button><button role="tab" aria-selected="false" data-nx22-tab="impressoes">Impressões</button><button role="tab" aria-selected="false" data-nx22-tab="franquia">Franquia</button><button role="tab" aria-selected="false" data-nx22-tab="recomendacoes">Recomendações</button></div></section>
      </nav>
    ...
    const impressions=()=>'<section class="nx22-full" id="nx22Impressions"><div class="nx22-panel-loading"><i></i><i></i><span>Carregando impressões…</span></div></section>';
    const activity=()=>'<section class="nx22-full nx22-activity-card"><div class="nx22-activity-head"><div><small>COMUNIDADE</small><h2>Atividade</h2></div><span>Como os membros estão acompanhando</span></div><div id="nx22Activity" class="nx22-activity-body" data-state="loading"><div class="nx22-panel-loading"><i></i><i></i><span>Carregando atividade…</span></div></div></section>';
    const general=()=>{const preview=synopsisPreview(m);return `${overviewSchedule}<div class="nx22-layout"><main><section class="nx22-section"><div class="nx22-section-head"><div><small>HISTÓRIA</small><h2>Sinopse</h2></div>${preview.pending?'<span class="nx22-translation-note" id="nx22SynopsisNote">Traduzindo…</span>':''}</div><p class="nx22-synopsis" id="nx22Synopsis">${esc(preview.text)}</p></section>${akira}${trailer}${watch}${chars.length?`<section class="nx22-section"><div class="nx22-section-head"><div><small>ELENCO</small><h2>Personagens principais</h2></div><button type="button" data-nx22-jump="elenco">Ver todos</button></div><div class="nx22-people">${chars.slice(0,10).map(personCard).join('')}</div></section>`:''}</main><aside><section><h3>Ficha técnica</h3><div class="nx22-info-card">${infoRows(m)}</div></section><section><h3>Outros títulos</h3>${altsHtml}</section></aside></div>${activity()}${impressions()}`};
    const panels={geral:general,aberturas:openings,elenco:cast,impressoes:impressions,franquia:franchise,recomendacoes:recommendations};
    const tabLabels={geral:'Visão geral',aberturas:'Aberturas & encerramentos',elenco:'Personagens & equipe',impressoes:'Impressões',franquia:'Franquia',recomendacoes:'Recomendações'};
    ...
    function show(k){const panel=root.querySelector('#nx22Panel');if(!panel)return;let activeButton=null;root.querySelectorAll('[data-nx22-tab]').forEach(b=>{const active=b.dataset.nx22Tab===k;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active));if(active)activeButton=b});const current=root.querySelector('[data-nx22-current-tab]');if(current)current.textContent=tabLabels[k]||tabLabels.geral;panel.innerHTML=(panels[k]||general)();panel.classList.remove('is-entering');if(panelReady)requestAnimationFrame(()=>panel.classList.add('is-entering'));else panelReady=true;wirePanel(k);closeTabs();if(innerWidth>720)activeButton?.scrollIntoView?.({behavior:'smooth',block:'nearest',inline:'center'});dispatchEvent(new CustomEvent('aninexus:detail-panel',{detail:{key:k,type:media.type,id:m.id,host:(k==='geral'||k==='impressoes')?root.querySelector('#nx22Impressions'):panel}}));}
